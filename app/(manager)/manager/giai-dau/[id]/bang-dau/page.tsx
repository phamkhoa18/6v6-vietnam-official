                    ...
                    <div className="bg-[#F0F7FF] rounded-lg p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-y-2 mb-6">
                        <div>
                            ...
                            <div className="text-gray-500 text-xs flex flex-wrap items-center gap-4 sm:gap-6">
                                <span>Hình thức thi đấu: <span className="text-gray-900 font-semibold">{tournament?.format === 'round_robin' ? 'Vòng tròn' : tournament?.format === 'group_stage' ? 'Vòng bảng' : "Loại trực tiếp"}</span></span>
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        <div className="flex gap-2">
                            ...
                            <button
                                onClick={handleCopy}
                                className="w-[52px] h-[52px] flex items-center justify-center bg-white border border-gray-200 rounded-xl hover:border-blue-600 hover:bg-blue-50 transition-all group"
                            >
                                {copied ? <Check className="w-5 h-5 text-emerald-500" /> : <Copy className="w-5 h-5 text-gray-400 group-hover:text-blue-600" />}
                            </button>
                        </div>
                    </div>
                </div>
        ...
        try {
            ...
            if (res.success) {
                ...
                const t = rawTeams.map((team: any) => {
                    ...
                    if (r) {
                        return { ...team, player1Name: r.playerName, player2Name: r.player2Name };
                    }
                    return team;
                });
            ...
            <div className="max-w-3xl mx-auto">
                ...
                <div className="text-center mb-8">
                    ...
                    <p className="text-sm text-gray-400">
                        {teams.length} đội tham gia · {tournament?.format === 'round_robin' ? 'Vòng tròn (Round Robin)' : tournament?.format === 'group_stage' ? 'Vòng bảng + Loại trực tiếp' : 'Loại trực tiếp (Single Elimination)'}
                    </p>
                </div>
                    ...
                    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm mb-6 overflow-hidden">
                        ...
                        <div className="p-4 border-b border-gray-100 flex flex-col sm:flex-row items-start sm:items-center gap-3">
                            <div className="flex-1 relative w-full sm:w-auto">
                                ...
                                <input
                                    type="text"
                                    placeholder="Tìm VĐV theo tên, đội..."
                                    value={searchTerm}
    ...
    const loadData = async () => {
        ...
        try {
            ...
            if (tRes.success) {
                ...
                const rawTeams = tRes.data?.teams || [];
                setTeams(rawTeams);
            }

            // Group matches by round
            const rawMatches = bRes.success ? (bRes.data?.matches || bRes.data || []) : [];

            // Compute bracketPosition and nextMatch (6v6 DB doesn't store these)
            ...
            const sortedRoundKeys = Object.keys(roundGroups).map(Number).sort((a, b) => a - b);
            
            // For each round, assign bracketPosition.y based on index within that round
            sortedRoundKeys.forEach((roundNum) => {
                const roundMatches = roundGroups[roundNum];
                roundMatches.sort((a: any, b: any) => (a.matchNumber || 0) - (b.matchNumber || 0));
                roundMatches.forEach((m: any, idx: number) => {
                    if (!m.bracketPosition || m.bracketPosition.y === undefined) {
                        m.bracketPosition = { x: roundNum - 1, y: idx };
                    }
                });
            });

            // Assign nextMatch references for connector lines
            sortedRoundKeys.forEach((roundNum, rIdx) => {
                if (rIdx >= sortedRoundKeys.length - 1) return;
                const nextRoundNum = sortedRoundKeys[rIdx + 1];
                const currentMatches = roundGroups[roundNum];
                const nextMatches = roundGroups[nextRoundNum] || [];
                currentMatches.forEach((m: any, idx: number) => {
                    const nextIdx = Math.floor(idx / 2);
                    if (nextMatches[nextIdx]) {
                        m.nextMatch = nextMatches[nextIdx]._id || nextMatches[nextIdx].id || true;
                    }
                });
            });
        ...
        <div className="flex flex-col h-[calc(100vh-120px)] space-y-4">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between bg-white px-4 py-2.5 rounded-lg border border-gray-200 gap-3">
                <div className="flex items-center gap-2.5">
                    ...
                    <div>
                        <h1 className="text-sm font-bold text-gray-900 leading-tight">{isRoundRobin ? 'Bảng xếp hạng & Lịch đấu' : 'Sơ đồ thi đấu'}</h1>
                        <p className="text-[11px] text-gray-400 font-medium flex items-center gap-1 mt-px">
                            <span className="w-1 h-1 rounded-full bg-emerald-500" />
                            {totalTeams} trận · {isRoundRobin ? `${teams.length} đội · Vòng tròn` : `${totalRounds} vòng`}
                        </p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-1.5 flex-wrap">
                    ...
                    {isOwner && !isRoundRobin && !isGroupStage && (
                        <>
                            <Button
                    ...
                    {isOwner && (isRoundRobin || isGroupStage) && filteredRounds.length > 0 && (
                        <Button
                            onClick={async () => {
                                if (!confirm('Bạn có chắc muốn tạo lại lịch đấu? Toàn bộ kết quả sẽ bị xóa.')) return;
                                const res = await tournamentAPI.generateBrackets(id, { force: true });
                                if (res.success) {
                                    toast.success('Đã tạo lại lịch đấu!');
                                    loadData();
                                } else {
                                    toast.error(res.message || 'Lỗi tạo lại');
                                }
                    ...
                    <Button
                        ...
                        className="h-8 px-2.5 rounded-md bg-blue-600 text-white hover:bg-blue-600-light font-semibold flex items-center gap-1 text-[11px]"
                    >
            ...
            {filteredRounds.length === 0 ? (
                ...
                <div id="bracket-capture-area" className="flex-1 overflow-auto bg-[#FDFDFD] rounded-[24px] border border-gray-100 relative custom-scrollbar">
                    <div className="p-6 space-y-6">
                        ...
                        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                            <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h2 className="text-base font-extrabold text-gray-900">Bảng xếp hạng</h2>
                                        <p className="text-[11px] text-gray-400 font-medium">
                                            {teams.length} đội · {isRoundRobin ? 'Vòng tròn' : 'Vòng bảng'} · Thắng={tournament?.scoring?.pointsPerWin ?? 3}đ · Hòa={tournament?.scoring?.pointsPerDraw ?? 1}đ · Thua={tournament?.scoring?.pointsPerLoss ?? 0}đ
                                        </p>
                                    </div>
                                </div>
                            </div>
                                ...
                                <table className="w-full text-sm">
                                    <thead>
                                        <tr className="bg-gray-50/50 border-y border-gray-100">
                                            <th className="px-3 py-2.5 text-center text-[11px] font-semibold text-gray-500 w-10">#</th>
                                            <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-gray-500">Đội / VĐV</th>
                                            <th className="px-2 py-2.5 text-center text-[11px] font-semibold text-gray-500 w-10">Trận</th>
                                            <th className="px-2 py-2.5 text-center text-[11px] font-semibold text-gray-500 w-9">T</th>
                                            <th className="px-2 py-2.5 text-center text-[11px] font-semibold text-gray-500 w-9">H</th>
                                            <th className="px-2 py-2.5 text-center text-[11px] font-semibold text-gray-500 w-9">B</th>
                                            <th className="px-2 py-2.5 text-center text-[11px] font-semibold text-gray-500 w-10">BT</th>
                                            <th className="px-2 py-2.5 text-center text-[11px] font-semibold text-gray-500 w-10">BB</th>
                                            <th className="px-2 py-2.5 text-center text-[11px] font-semibold text-gray-500 w-10">HS</th>
                                            <th className="px-2 py-2.5 text-center text-[11px] font-semibold text-gray-500 w-24">Phong độ</th>
                                            <th className="px-3 py-2.5 text-center text-[11px] font-bold text-gray-900 w-14">Điểm</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-gray-50">
                                        {[...teams]
                                            ...
                                            .map((team, idx) => {
                                                const s = team.stats || {};
                                                const rank = idx + 1;
                                                const isTop = rank <= (tournament?.scoring?.advancePerGroup || 2);
                                                const gd = s.goalDifference || 0;
                                                const form = (s.form || []).slice(-5);

                                                return (
                                                    <tr key={team._id} className={`hover:bg-gray-50/70 transition-colors ${isTop ? 'bg-emerald-50/20' : ''}`}>
                                                        {/* Rank */}
                                                        <td className="px-3 py-3.5 text-center">
                                                            <span className={`text-[13px] font-medium ${
                                                                rank === 1 ? 'text-amber-500 font-bold' :
                                                                rank === 2 ? 'text-gray-400 font-bold' :
                                                                rank === 3 ? 'text-amber-700 font-bold' :
                                                                'text-gray-500'
                                                            }`}>{rank}</span>
                                                        </td>
                                                        {/* Team */}
                                                        <td className="px-4 py-3.5">
                                                            <div className="flex items-center gap-3">
                                                                {team.logo ? (
                                                                    <img src={team.logo} alt="" className="w-7 h-7 rounded-full object-cover border border-gray-200 flex-shrink-0" />
                                                                ) : (team.captain?.avatar || team.captain?.personalPhoto) ? (
                                                                    <img src={team.captain.avatar || team.captain.personalPhoto} alt="" className="w-7 h-7 rounded-full object-cover border border-gray-200 flex-shrink-0" />
                                                                ) : (
                                                                    <div className="w-7 h-7 rounded-full bg-gray-100 flex items-center justify-center text-[11px] font-semibold text-gray-500 flex-shrink-0 border border-gray-200">
                                                                        {(team.name || '?').charAt(0).toUpperCase()}
                                                                    </div>
                                                                )}
                                                                <div className="min-w-0">
                                                                    <span className="text-[13px] font-medium text-gray-900 leading-snug">
                                                                        {tournament?.gameMode === '2v2' || tournament?.gameMode === '3v3' 
                                                                            ? (team.members?.map((m: any) => m.user?.name).filter(Boolean).join(" / ") || team.captain?.name || team.name)
                                                                            : (team.name || team.captain?.name)}
                                                                    </span>
                                                                    {tournament?.gameMode !== '2v2' && tournament?.gameMode !== '3v3' && team.name && team.captain?.name && (
                                                                        <span className="text-[11px] text-gray-500 truncate block mt-0.5">{team.captain?.name}{team.shortName ? ` (${team.shortName})` : ''}</span>
                                                                    )}
                                                                </div>
                                                            </div>
                                                        </td>
                                                        {/* Stats */}
                                                        <td className="px-2 py-3.5 text-center text-[13px] font-medium text-gray-500">{s.played || 0}</td>
                                                        <td className="px-2 py-3.5 text-center text-[13px] font-semibold text-gray-700">{s.wins || 0}</td>
                                                        <td className="px-2 py-3.5 text-center text-[13px] font-semibold text-gray-700">{s.draws || 0}</td>
                                                        <td className="px-2 py-3.5 text-center text-[13px] font-semibold text-gray-700">{s.losses || 0}</td>
                                                        <td className="px-2 py-3.5 text-center text-[13px] font-medium text-gray-400">{s.goalsFor || 0}</td>
                                                        <td className="px-2 py-3.5 text-center text-[13px] font-medium text-gray-400">{s.goalsAgainst || 0}</td>
                                                        <td className="px-2 py-3.5 text-center">
                                                            <span className={`text-[13px] font-semibold ${gd > 0 ? 'text-gray-900' : gd < 0 ? 'text-gray-900' : 'text-gray-400'}`}>
                                                                {gd > 0 ? `+${gd}` : gd}
                                                            </span>
                                                        </td>
                                                        {/* Form */}
                                                        <td className="px-2 py-3.5">
                                                            <div className="flex items-center justify-center gap-1">
                                                                {form.map((f: string, fi: number) => (
                                                                    <span key={fi} className={`w-4 h-4 rounded-full text-[9px] font-bold flex items-center justify-center ${
                                                                        f === 'W' ? 'bg-emerald-500 text-white' :
                                                                        f === 'D' ? 'bg-gray-300 text-gray-700' :
                                                                        f === 'L' ? 'bg-red-500 text-white' :
                                                                        f === 'PW' ? 'bg-blue-500 text-white' :
                                                                        f === 'PL' ? 'bg-orange-500 text-white' :
                                                        ...
                                                        {/* Points */}
                                                        <td className="px-3 py-3.5 text-center">
                                                            <span className="text-[14px] font-extrabold text-gray-900">{s.points || 0}</span>
                                                        </td>
                                                    </tr>
                                                );
                                            })}
                        ...
                        {/* === LỊCH THI ĐẤU (Match List by Round) === */}
                        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                            <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h2 className="text-base font-extrabold text-gray-900">Lịch thi đấu</h2>
                                        <p className="text-[11px] text-gray-400 font-medium">
                                            {bracketRounds.reduce((s, r) => s + r.matches.length, 0)} trận · {bracketRounds.length} vòng
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            {/* Round Tabs */}
                            <div className="px-5 py-3 border-b border-gray-100 flex items-center gap-2 overflow-x-auto custom-scrollbar">
                                {bracketRounds.map((round, rIdx) => {
                                    const roundNum = round.matches[0]?.round || rIdx + 1;
                                    const completedInRound = round.matches.filter((m: any) => m.status === 'completed').length;
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-bold ${
                                                ...
                                                completedInRound === totalInRound && totalInRound > 0 ? 'bg-emerald-100 text-emerald-600' :
                            ...
                            <div className="p-5">
                                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
                                    {bracketRounds
                                        ...
                                        .map((match: any) => {
                                            ...
                                            const homeName = match.homeTeam?.name || match.homeTeam?.shortName || match.p1?.name || 'TBD';
                                            const awayName = match.awayTeam?.name || match.awayTeam?.shortName || match.p2?.name || 'TBD';
                                            const homePlayer = match.homeTeam?.player1 || match.homeTeam?.captain?.name || match.p1?.name || homeName;
                                            const awayPlayer = match.awayTeam?.player1 || match.awayTeam?.captain?.name || match.p2?.name || awayName;
                                            ...
                                            return (
                                                <motion.div
                                                    ...
                                                    className={`rounded-xl border cursor-pointer transition-all overflow-hidden ${
                                                        ...
                                                        isLive ? 'border-red-200 bg-red-50/30 shadow-sm ring-1 ring-red-100' :
                                                ...
                                                >
                                                    ...
                                                    <div className={`px-3 py-1.5 flex items-center justify-between text-[10px] font-bold ${
                                                        ...
                                                        <span className="flex items-center gap-1">
                                                            ...
                                                            {isCompleted ? '✓ Kết thúc' : isLive ? 'LIVE' : 'Chưa đá'}
                                                        </span>
                                                    ...
                                                    <div className="p-3">
                                                        {/* Home */}
                                                        <div className={`flex items-center justify-between py-1.5 ${homeWin ? 'text-gray-900' : 'text-gray-600'}`}>
                                                            <div className="flex items-center gap-2 min-w-0 flex-1">
                                                                {(match.homeTeam?.logo || match.homeTeam?.captain?.avatar) ? (
                                                                    <img src={match.homeTeam.logo || match.homeTeam.captain?.avatar} alt="" className="w-6 h-6 rounded-md object-cover flex-shrink-0" />
                                                                ) : (
                                                                    <div className="w-6 h-6 rounded-md bg-blue-100 flex items-center justify-center text-[9px] font-bold text-blue-600 flex-shrink-0">
                                                                        {homePlayer.charAt(0).toUpperCase()}
                                                                    </div>
                                                                )}
                                                                <span className={`text-sm truncate ${homeWin ? 'font-bold' : 'font-medium'}`}>{homePlayer}</span>
                                                            </div>
                                                            <span className={`text-lg tabular-nums ml-2 ${homeWin ? 'font-black text-emerald-600' : isDraw && isCompleted ? 'font-bold text-amber-500' : 'font-semibold text-gray-400'}`}>
                                                        ...
                                                        {/* Away */}
                                                        <div className={`flex items-center justify-between py-1.5 ${awayWin ? 'text-gray-900' : 'text-gray-600'}`}>
                                                            <div className="flex items-center gap-2 min-w-0 flex-1">
                                                                {(match.awayTeam?.logo || match.awayTeam?.captain?.avatar) ? (
                                                                    <img src={match.awayTeam.logo || match.awayTeam.captain?.avatar} alt="" className="w-6 h-6 rounded-md object-cover flex-shrink-0" />
                                                                ) : (
                                                                    <div className="w-6 h-6 rounded-md bg-rose-100 flex items-center justify-center text-[9px] font-bold text-rose-600 flex-shrink-0">
                                                                        {awayPlayer.charAt(0).toUpperCase()}
                                                                    </div>
                                                                )}
                                                                <span className={`text-sm truncate ${awayWin ? 'font-bold' : 'font-medium'}`}>{awayPlayer}</span>
                                                            </div>
                                                            <span className={`text-lg tabular-nums ml-2 ${awayWin ? 'font-black text-emerald-600' : isDraw && isCompleted ? 'font-bold text-amber-500' : 'font-semibold text-gray-400'}`}>
                                                        ...
                                                        {isDraw && (
                                                            <div className="mt-1.5 text-center">
                                                                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-bold bg-amber-100 text-amber-600 border border-amber-200">
                                                                    Hòa
                                                                </span>
                                                            </div>
                                                        )}
                                ...
                                {bracketRounds.filter(r => (r.matches[0]?.round || 1) === activeRound).flatMap(r => r.matches).length === 0 && (
                                    <div className="text-center py-8 text-gray-400 text-sm">
                                        Chưa có trận đấu trong vòng này
                                    </div>
                                )}
                            </div>
                        </div>
