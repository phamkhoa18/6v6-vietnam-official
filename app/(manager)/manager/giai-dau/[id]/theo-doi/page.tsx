...
export default function TheoDoiPage() {
    ...
    const stats = [
        { label: "Tổng lượt xem", value: tournament?.views?.toLocaleString() || "0", change: "", icon: Eye, bg: "bg-red-50", color: "text-red-500" },
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                ...
                <button
                    onClick={loadData}
                    className="flex items-center gap-1.5 text-xs text-efb-text-muted hover:text-efb-red transition-colors"
                >
            ...
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="bg-white rounded-2xl border border-gray-100 p-6">
                <h3 className="text-sm font-semibold text-efb-dark mb-4 flex items-center gap-2">
                    <BarChart3 className="w-4 h-4 text-efb-red" />
                    Tiến độ giải đấu
                </h3>
                <div className="space-y-3">
                    ...
                    <div className="w-full bg-gray-100 rounded-full h-2">
                        <div
                            className="bg-gradient-to-r from-red-500 to-rose-500 rounded-full h-2 transition-all duration-700"
