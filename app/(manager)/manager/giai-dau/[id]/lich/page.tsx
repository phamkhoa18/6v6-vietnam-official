...
export default function LichThiDauPage() {
    ...
    const loadData = async () => {
        ...
        try {
            ...
            if (mRes.success) {
                const rawMatches = mRes.data?.matches || [];
                setMatches(rawMatches);

                // API returns flat matches array — group by roundName for display
                let fetchedRounds: Record<string, any[]> = mRes.data?.rounds || {};

                // If rounds is empty but matches exist, build rounds from flat array
                if (Object.keys(fetchedRounds).length === 0 && rawMatches.length > 0) {
                    fetchedRounds = {};
                    rawMatches.forEach((m: any) => {
                        const roundKey = m.roundName || (m.group ? `Bảng ${m.group}` : `Vòng ${m.round || 1}`);
                        if (!fetchedRounds[roundKey]) fetchedRounds[roundKey] = [];
                        fetchedRounds[roundKey].push(m);
                    });
                }
    ...
    const filterMatch = (m: any) => {
        ...
        if (matchSearchTerm.trim()) {
            ...
            const homeCap = (m.homeTeam?.captain?.name || '').toLowerCase();
            const awayCap = (m.awayTeam?.captain?.name || '').toLowerCase();
            ...
            return (
                ...
                homeCap.includes(s) || awayCap.includes(s) ||
                matchNum.includes(s) ||
        ...
        <div className="space-y-4 max-w-full overflow-hidden pb-20">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between bg-white px-4 py-3 border-b border-gray-100 gap-4">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                    <Button variant="outline" size="sm" className="text-blue-600 border-blue-200 bg-blue-50 hover:bg-blue-100 h-8 px-3 rounded text-xs font-semibold" onClick={() => router.push(`/manager/giai-dau/${id}/bang-dau`)}>
                            ...
                            <div key={roundName}>
                                ...
                                <div className="divide-y divide-gray-100">
                                    {roundMatches.filter((m: any) => m.status !== 'walkover' && m.status !== 'bye').filter(filterMatch).map((m: any, index: number) => {
                                        const isSmallTeamMode = tournament?.gameMode === '2v2' || tournament?.gameMode === '3v3';
                                        const is1v1 = tournament?.gameMode === '1v1';
                                        
                                        let homeName = m.homeTeam?.name || "Tự do";
                                        let awayName = m.awayTeam?.name || "Tự do";
                                        
                                        // p1Name is used for the wide column on desktop and the MAIN line on mobile
                                        let p1Name = "";
                                        let p2Name = "";
                                        const p1Sub = "";
                                        const p2Sub = "";
                                        ...
                                        if (isSmallTeamMode) {
                                            homeName = m.homeTeam?.shortName ? m.homeTeam.shortName : "Đội nhà";
                                            awayName = m.awayTeam?.shortName ? m.awayTeam.shortName : "Đội khách";
                                            p1Name = m.homeTeam?.player1 || m.homeTeam?.name || "—";
                                            p2Name = m.awayTeam?.player1 || m.awayTeam?.name || "—";
                                        } else if (is1v1) {
                                            homeName = m.p1?.name || "Tự do";
                                            awayName = m.p2?.name || "Tự do";
                                            p1Name = homeName;
                                            p2Name = awayName;
                                        } else {
                                            // 6v6
                                            homeName = m.homeTeam?.name || "Tự do";
                                            awayName = m.awayTeam?.name || "Tự do";
                                            p1Name = homeName; // Mobile uses p1Name, so make it the team name
                                            p2Name = awayName;
                                        }
                                                ...
                                                <div className={`hidden lg:grid grid-cols-12 gap-4 items-center py-2 px-4 transition-colors ${swap.isSwapMode ? 'hover:bg-transparent' : 'hover:bg-gray-50'}`}>
                                                    ...
                                                    <div className="col-span-2 flex flex-col gap-1.5">
                                                        <div 
                                                            ...
                                                            onClick={isSwappable && hTeamId ? () => swap.handleTeamSelect(hTeamId, m.homeTeam?.captain?.name || m.homeTeam?.name || homeName) : undefined}
                                                        >
                                                        ...
                                                        <div 
                                                            ...
                                                            onClick={isSwappable && aTeamId ? () => swap.handleTeamSelect(aTeamId, m.awayTeam?.captain?.name || m.awayTeam?.name || awayName) : undefined}
                                                        >
                                                    ...
                                                    <div className="col-span-4 flex flex-col gap-1.5 text-[13px] font-medium">
                                                        {(!is1v1 && !isSmallTeamMode) ? (
                                                            // For 6v6, show captain name in the sub column
                                                            <>
                                                                <div className={`${isCompleted ? (isHomeWin ? "font-bold text-gray-900" : "text-gray-400 line-through") : "text-purple-600"}`}>{m.homeTeam?.captain?.name || "—"}</div>
                                                                <div className={`${isCompleted ? (isAwayWin ? "font-bold text-gray-900" : "text-gray-400 line-through") : "text-purple-600"}`}>{m.awayTeam?.captain?.name || "—"}</div>
                                                            </>
                                                        ) : (
                                                            <>
                    ...
                    <div className="p-4 sm:p-6 overflow-y-auto space-y-4 flex-1">
                        {viewingSubmissions?.resultSubmissions?.length > 0 ? (() => {
                            ...
                            const renderSubmission = (sub: any, idx: number, isHistory: boolean = false) => {
                                ...
                                const displayName = userData.name || submitterTeam?.captain?.name || submitterTeam?.name || "VĐV";
                                        ...
                                        <div className="text-center">
                                            <div className="mb-1">
                                                <div className="flex items-center justify-center gap-1 flex-wrap">
                                                    ...
                                                    <span className="text-[10px] font-bold text-gray-600 truncate">{viewingSubmissions.homeTeam?.captain?.name || viewingSubmissions.homeTeam?.name || "Đội nhà"}</span>
                                                </div>
                                        ...
                                        <div className="text-center">
                                            <div className="mb-1">
                                                <div className="flex items-center justify-center gap-1 flex-wrap">
                                                    ...
                                                    <span className="text-[10px] font-bold text-gray-600 truncate">{viewingSubmissions.awayTeam?.captain?.name || viewingSubmissions.awayTeam?.name || "Đội khách"}</span>
                                                </div>
...
function EditMatchModal({ match, tournament, onClose, onSaved }: { match: any; tournament: any; onClose: () => void; onSaved: () => void }) {
    ...
    const formatNameStr = (team: any, pFallback: any) => {
        const p1 = team?.captain?.name || team?.name || pFallback?.name || "Tự do";
        const p2 = "";
        return `${p1}${p2}`;
    };
                    ...
                    <div className="bg-[#F0F7FF] rounded-lg p-3 sm:p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-y-1 mb-4 sm:mb-6">
                        <div>
                            ...
                            <div className="text-gray-500 text-xs flex flex-wrap items-center gap-3 sm:gap-6">
                                <span>Hình thức: <span className="text-gray-900 font-semibold">{tournament?.format === 'round_robin' ? 'Vòng tròn' : tournament?.format === 'group_stage' ? 'Vòng bảng' : "Loại trực tiếp"}</span></span>
                                        ...
                                        <div className="flex items-center gap-2 min-w-0">
                                            ...
                                            <div className="min-w-0">
                                                <div className="flex items-center gap-1 flex-wrap">
                                                    ...
                                                    <span className="text-[13px] font-semibold text-gray-900 leading-tight">{match.homeTeam?.captain?.name || match.homeTeam?.name || match.homeTeam?.player1 || match.p1?.name || "Tự do"}</span>
                                                </div>
                                        ...
                                        <div className="flex items-center gap-2 min-w-0">
                                            ...
                                            <div className="min-w-0">
                                                <div className="flex items-center gap-1 flex-wrap">
                                                    ...
                                                    <span className="text-[13px] font-semibold text-gray-900 leading-tight">{match.awayTeam?.captain?.name || match.awayTeam?.name || match.awayTeam?.player1 || match.p2?.name || "Tự do"}</span>
                                                </div>
                            ...
                            <div className="flex w-full max-w-lg border border-orange-200 rounded-md overflow-hidden bg-white">
                                ...
                                >
                                    <div className="flex items-center gap-1.5">
                                        ...
                                        <span>{match.homeTeam?.captain?.name || match.homeTeam?.name || match.p1?.name || "Tự do"}</span>
                                    </div>
                                ...
                                >
                                    <div className="flex items-center gap-1.5">
                                        ...
                                        <span>{match.awayTeam?.captain?.name || match.awayTeam?.name || match.p2?.name || "Tự do"}</span>
                                    </div>
                                    ...
                                    <div key={idx} className="rounded-xl bg-white border border-gray-200 shadow-sm overflow-hidden">
                                        ...
                                        <div className="flex items-center gap-3 mb-2 flex-wrap">
                                            <div className="flex items-center gap-1.5">
                                                ...
                                                <span className="text-xs font-bold text-gray-700">{match.homeTeam?.captain?.name || match.homeTeam?.name || "Đội nhà"}</span>
                                            </div>
                                            <div className="flex items-center gap-2">
                                            ...
                                            <div className="flex items-center gap-1.5">
                                                ...
                                                <span className="text-xs font-bold text-gray-700">{match.awayTeam?.captain?.name || match.awayTeam?.name || "Đội khách"}</span>
                                            </div>
                                        </div>
