        ...
        <div className="space-y-6">
            ...
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-2xl border border-gray-100 p-5">
                <div className="flex gap-3">
                    <div className="w-9 h-9 rounded-lg bg-red-50 flex items-center justify-center flex-shrink-0">
                        <MessageSquare className="w-4 h-4 text-efb-red" />
                    </div>
                    <textarea
                        className="flex-1 min-h-[60px] px-3 py-2 rounded-lg border border-gray-200 bg-white text-sm resize-none focus:border-efb-red focus:ring-1 focus:ring-efb-red/20 outline-none"
                ...
                <div className="flex justify-end mt-3">
                    <Button
                        className="bg-efb-red text-white hover:bg-efb-red/90 font-semibold text-xs h-9 px-4 rounded-lg"
            ...
            {isLoading ? (
                <div className="flex items-center justify-center py-16">
                    <Loader2 className="w-6 h-6 animate-spin text-efb-red" />
                </div>
            ) : feedbacks.length === 0 ? (
                    ...
                    {feedbacks.map((fb: any, i: number) => (
                        ...
                        >
                            <div className="flex items-center gap-2.5 mb-2">
                                <div className="w-7 h-7 rounded-full bg-gradient-to-br from-red-500 to-rose-600 flex items-center justify-center text-white text-[10px] font-bold">
                            ...
                            {fb.likes !== undefined && (
                                <div className="flex items-center gap-1.5 ml-[36px] mt-2">
                                    <button className="flex items-center gap-1 text-xs text-efb-text-muted hover:text-efb-red transition-colors">
                                        <ThumbsUp className="w-3.5 h-3.5" /> {fb.likes || 0}
                                    </button>
                                </div>
                            )}
                            {fb.reply && (
                                <div className="ml-[36px] mt-3 bg-red-50/50 rounded-lg p-3 border border-red-100">
                                    <div className="text-[11px] text-efb-red font-semibold mb-1">Phản hồi từ BTC</div>
