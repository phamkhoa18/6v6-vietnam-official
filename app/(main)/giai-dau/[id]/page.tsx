...
import {
    ...
    Gamepad2, Award, FileText, UserPlus, Clock, Shield, Swords, Camera, MapPinned, Loader2, Globe, CheckCircle2, Eye, Ban, DollarSign, Phone, Mail, MessageCircle,
    LogIn, AlertCircle, Info, X, Watch, CreditCard, Upload, ExternalLink, Wallet, Image as ImageIcon, User,
    Zap, Target, ArrowRight, Search, Maximize2, Minimize2, ChevronDown, PlaySquare
} from "lucide-react";
...
const tabs = [
    ...
    { key: "videos", label: "Video", icon: PlaySquare },
];
...
function extractYouTubeId(url: string): string | null {
    ...
    const iframeMatch = url.match(/src=["'](?:https?:)?\/\/(?:www\.)?youtube\.com\/embed\/([a-zA-Z0-9_-]{11})/);
                ...
                >
                    ...
                    <div className="p-1.5 flex flex-col">
                        <span className="text-[8px] text-gray-400 font-bold text-center mb-0.5">
                            ...
                            {remainingTeam?.seed != null && remainingTeam.seed > 0 && <span className="text-[8px] font-bold text-efb-red bg-red-50 border border-red-200 px-1 py-px rounded mr-1" title={`Hạt giống số ${remainingTeam.seed}`}>Seed {remainingTeam.seed}</span>}
                            {woName}
                        </span>
    ...
    return (
        <div className="flex items-center relative z-20 w-[200px]">
            ...
            >
                ...
                <div className={`p-1.5 flex flex-col ${homeWin ? "bg-red-50/20" : ""} ${isLive ? 'mt-[10px]' : ''}`}>
                    <span className="text-[8px] text-gray-400 font-bold text-center mb-0.5">
                        
                        {match.homeTeam?.seed != null && match.homeTeam.seed > 0 && <span className="text-[8px] font-bold text-efb-red bg-red-50 border border-red-200 px-1 py-px rounded mr-1" title={`Hạt giống số ${match.homeTeam.seed}`}>Seed {match.homeTeam.seed}</span>}
                        {homeName}
                    </span>
                    <div className="flex justify-between items-center px-1">
                        <div className="flex items-center min-w-0 pr-1 leading-[1.1] gap-0.5">
                            <span className={`truncate text-[11px] ${homeWin ? "text-efb-red-light font-bold" : "text-gray-800 font-bold"} ${!match.homeTeam && !match.p1 ? "text-gray-400 italic font-medium" : ""}`}>
                        ...
                        <span className={`text-[12px] tabular-nums ml-1 flex-shrink-0 ${homeWin ? "text-efb-red font-bold" : "text-gray-400 font-semibold"}`}>{homeScore}</span>
                    </div>
                </div>

                <div className="h-px bg-[#E2E8F0] w-full" />

                <div className={`p-1.5 flex flex-col ${awayWin ? "bg-red-50/20" : ""}`}>
                    <span className="text-[8px] text-gray-400 font-bold text-center mb-0.5">
                        
                        {match.awayTeam?.seed != null && match.awayTeam.seed > 0 && <span className="text-[8px] font-bold text-efb-red bg-red-50 border border-red-200 px-1 py-px rounded mr-1" title={`Hạt giống số ${match.awayTeam.seed}`}>Seed {match.awayTeam.seed}</span>}
                        {awayName}
                    </span>
                    <div className="flex justify-between items-center px-1">
                        <div className="flex items-center min-w-0 pr-1 leading-[1.1] gap-0.5">
                            <span className={`truncate text-[11px] ${awayWin ? "text-efb-red-light font-bold" : "text-gray-800 font-bold"} ${!match.awayTeam && !match.p2 ? "text-gray-400 italic font-medium" : ""}`}>
                        ...
                        <span className={`text-[12px] tabular-nums ml-1 flex-shrink-0 ${awayWin ? "text-efb-red font-bold" : "text-gray-400 font-semibold"}`}>{awayScore}</span>
                    </div>
                </div>
            </motion.div>
        </div>
...
const MatchDetailViewModal = ({ match, tournament, onClose, user, myRegistration }: { match: any; tournament: any; onClose: () => void; user?: any; myRegistration?: any }) => {
    ...
    const getPlayerInfo = (team: any, fallback: any) => {
        const membersExceptCaptain = (team?.members || []).filter((m: any) => m.user && m?.user?._id !== team?.captain?._id);
        const t2 = membersExceptCaptain[0]?.user;
        const t3 = membersExceptCaptain[1]?.user;

        const mainName = (tournament?.gameMode === '2v2' || tournament?.gameMode === '3v3') 
            ? (team?.captain?.name || team?.player1) 
            : (team?.name || team?.captain?.name || team?.player1);

        return {
            name: mainName || fallback?.name || "Chờ xác định",
            teamName: (tournament?.gameMode === '2v2' || tournament?.gameMode === '3v3') ? team?.shortName : (team?.name || team?.shortName || ""),
            userId: team?.userId || team?.user || team?.captain?._id || team?.captain,
            avatar: (tournament?.teamSize >= 2 && team?.teamLineupPhoto) ? team.teamLineupPhoto : (team?.personalPhoto || team?.avatar || team?.logo || team?.captain?.avatar || ''),
            facebookName: team?.facebookName || team?.captain?.facebookName,
            facebookLink: team?.facebookLink || team?.captain?.facebookLink,
            player2Name: team?.player2Name || team?.player2 || t2?.name,
            player2FacebookName: team?.player2FacebookName || t2?.facebookName,
            player2FacebookLink: team?.player2FacebookLink || t2?.facebookLink,
            player3Name: team?.player3Name || team?.player3 || t3?.name,
            player3FacebookName: team?.player3FacebookName || t3?.facebookName,
            player3FacebookLink: team?.player3FacebookLink || t3?.facebookLink,
        };
    };
    ...
    const renderScoreAvatar = (info: typeof home) => {
        ...
        return (
            <div className="flex-shrink-0">
                ...
                <div className={`w-11 h-11 rounded-full bg-gradient-to-br from-efb-red/20 to-rose-500/20 items-center justify-center border-2 border-white/10 ${hasAvatar ? 'hidden' : 'flex'}`}>
            ...
            >
                ...
                <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-100 flex-shrink-0 bg-white">
                    <div className="flex items-center gap-2">
                        <div className="w-7 h-7 rounded-lg bg-efb-red/10 flex items-center justify-center">
                            <Swords className="w-3.5 h-3.5 text-efb-red" />
                        </div>
                        <div>
                ...
                <div className="flex-1 overflow-y-auto">
                    {/* Score Card */}
                    <div className={`px-5 py-6 ${isCompleted ? 'bg-gradient-to-br from-slate-800 via-slate-900 to-gray-900' : isLive ? 'bg-gradient-to-br from-red-600 to-red-700' : 'bg-gradient-to-br from-red-50/50 to-rose-50/30'}`}>
                        ...
                        <div className="flex items-center justify-center gap-3 sm:gap-5">
                            ...
                            <div className="flex-shrink-0 text-center">
                                ...
                                <div className={`mt-1 text-[10px] font-semibold ${isDark ? '' : ''}`}>
                                    ...
                                    {!isCompleted && !isLive && <span className="text-red-400 flex items-center justify-center gap-1"><Clock className="w-3 h-3" />Chờ đấu</span>}
                                </div>
                            </div>
                        ...
                        {[
                            ...
                            <div key={idx} className={`px-4 py-3 ${isWin ? 'bg-emerald-50/40' : ''}`}>
                                ...
                                <div className="flex items-center gap-2.5">
                                    <div className="flex-shrink-0">
                                        {info.avatar ? (
                                            <img src={info.avatar} alt={info.name} className="w-10 h-10 rounded-full object-cover border-2 border-red-200 shadow-sm" onError={handleImgError} />
                                        ) : null}
                                        <div className={`w-10 h-10 rounded-full bg-gradient-to-br from-red-100 to-rose-100 items-center justify-center border-2 border-red-200 ${info.avatar ? 'hidden' : 'flex'}`}>
                                            <User className="w-4.5 h-4.5 text-efb-red" />
                                        </div>
                                    </div>
                                    <div className="min-w-0 flex-1">
                                        ...
                                        {(info.facebookLink || info.facebookName) && (
                                            <a href={info.facebookLink} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} className="mt-1 flex items-center gap-1 border border-red-200 bg-red-50 px-1.5 py-0.5 rounded-md hover:bg-red-100 transition-colors max-w-max">
                                                <Globe className="w-3 h-3 text-efb-red" />
                                                <span className="text-[10px] font-semibold text-efb-red-light truncate max-w-[120px]">{info.facebookName || 'Facebook'}</span>
                                            </a>
                                        )}
                                        ...
                                        {info.player2Name && (
                                            <div className="mt-2 pt-2 border-t border-gray-100 border-dashed">
                                                ...
                                                {(info.player2FacebookLink || info.player2FacebookName) && (
                                                    <a href={info.player2FacebookLink} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} className="mt-1 flex items-center gap-1 border border-red-200 bg-red-50 px-1.5 py-0.5 rounded-md hover:bg-red-100 transition-colors max-w-max">
                                                        <Globe className="w-3 h-3 text-efb-red" />
                                                        <span className="text-[10px] font-semibold text-efb-red-light truncate max-w-[120px]">{info.player2FacebookName || 'Facebook'}</span>
                                                    </a>
                                                )}
                                            </div>
                                        )}
                                        {info.player3Name && (
                                            <div className="mt-2 pt-2 border-t border-gray-100 border-dashed">
                                                <div className="flex items-center gap-1 flex-wrap">
                                                    <span className="text-[11px] font-semibold text-gray-700" style={{ wordBreak: 'break-word' }}>VĐV 3: {info.player3Name}</span>
                                                </div>
                                                {(info.player3FacebookLink || info.player3FacebookName) && (
                                                    <a href={info.player3FacebookLink} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} className="mt-1 flex items-center gap-1 border border-red-200 bg-red-50 px-1.5 py-0.5 rounded-md hover:bg-red-100 transition-colors max-w-max">
                                                        <Globe className="w-3 h-3 text-efb-red" />
                                                        <span className="text-[10px] font-semibold text-efb-red-light truncate max-w-[120px]">{info.player3FacebookName || 'Facebook'}</span>
                                                    </a>
                                                )}
                                            </div>
                                        )}
                                        
                                        {info.teamName && <p className="text-[10px] text-gray-500 mt-2 bg-gray-50 border border-gray-200 px-2 py-1 rounded inline-block font-bold shadow-sm" style={{ wordBreak: 'break-word' }}>{info.teamName}</p>}
                                    </div>
                                </div>
                            </div>
                        ))}
                    ...
                    <div className="px-5 py-4 space-y-4">
                                    ...
                                    {match.resultSubmissions.map((sub: any, idx: number) => {
                                        ...
                                        return (
                                            <div key={idx} className="rounded-xl border border-gray-200 overflow-hidden bg-white shadow-sm">
                                                <div className={`px-3.5 py-2.5 ${isFromHome ? 'bg-gradient-to-r from-red-50/50 to-rose-50/30 border-b border-red-100' : 'bg-gradient-to-r from-rose-50 to-pink-50 border-b border-rose-100'}`}>
                                                    <div className="flex items-center gap-2.5">
                                                        <div className="flex-shrink-0">
                                                            {displayAvatar ? (
                                                            ...
                                                            <div className={`w-8 h-8 rounded-full items-center justify-center text-sm font-bold ${isFromHome ? 'bg-red-100 text-efb-red-light border-2 border-red-200' : 'bg-rose-100 text-rose-700 border-2 border-rose-200'} ${displayAvatar ? 'hidden' : 'flex'}`}>
                                                                <User className="w-4 h-4" />
                                                            </div>
                                                        </div>
                                                        <div className="flex-1 min-w-0">
                                                            <div className="flex items-center gap-1 flex-wrap">
                                                                {isMe && <span className="text-[8px] font-bold text-white bg-red-500 px-1.5 py-px rounded-full flex-shrink-0">BẠN</span>}
                                                        ...
                                                        <div className="mt-3">
                                                            ...
                                                            <div className="grid grid-cols-3 gap-1.5">
                                                                {sub.screenshots.map((s: string, si: number) => (
                                                                    <div key={si} className="relative group cursor-pointer rounded-lg overflow-hidden border border-gray-100 hover:border-red-300 transition-all aspect-square" onClick={() => window.open(s, "_blank")}>
                        ...
                        {canSubmitResult && (
                            <div className="space-y-3">
                                ...
                                {!mySubmission && !showSubmitForm && (
                                    <button onClick={() => setShowSubmitForm(true)} className="w-full py-2.5 rounded-xl bg-gradient-to-r from-efb-red to-efb-red-light text-white font-bold text-sm shadow-lg shadow-red-500/20 hover:shadow-xl transition-all flex items-center justify-center gap-2">
                                        <Upload className="w-4 h-4" /> Gửi kết quả trận đấu
                                    </button>
                                )}
                                {showSubmitForm && (
                                    <div className="p-4 rounded-xl border-2 border-efb-red/30 bg-red-50/30">
...
export default function TournamentDetailClient({ params }: { params: Promise<{ id: string }> }) {
    const { id } = use(params);
    ...
    const [regForm, setRegForm] = useState({
        teamName: "", teamShortName: "", playerName: "", phone: "", email: "", notes: "",
        dateOfBirth: "", facebookName: "", facebookLink: "", province: "",
        personalPhoto: "", teamLineupPhoto: "",
        player2Name: "", player2FacebookName: "", player2FacebookLink: "",
        player3Name: "", player3FacebookName: "", player3FacebookLink: ""
    });
    ...
    const [selectedP2UserId, setSelectedP2UserId] = useState<string | null>(null);
    ...
    useEffect(() => {
        ...
        const delayDebounceFn = setTimeout(async () => {
            ...
            try {
                const params = new URLSearchParams({ q: p2SearchQuery, tournamentId: id });
                if (user?._id) params.set('excludeUserId', user._id);
                const res = await fetch(`/api/users/search-public?${params.toString()}`);
    ...
    // Player 3 Search State
    const [p3SearchQuery, setP3SearchQuery] = useState('');
    const [p3SearchResults, setP3SearchResults] = useState<any[]>([]);
    const [isP3Searching, setIsP3Searching] = useState(false);
    const [showP3Dropdown, setShowP3Dropdown] = useState(false);
    const [selectedP3UserId, setSelectedP3UserId] = useState<string | null>(null);

    useEffect(() => {
        if (!p3SearchQuery || p3SearchQuery.trim().length === 0) {
            setP3SearchResults([]);
            return;
        }
        const delayDebounceFn = setTimeout(async () => {
            setIsP3Searching(true);
            try {
                const params = new URLSearchParams({ q: p3SearchQuery, tournamentId: id });
                if (user?._id) params.set('excludeUserId', user._id);
                const res = await fetch(`/api/users/search-public?${params.toString()}`);
                const dt = await res.json();
                if (dt.success) setP3SearchResults(dt.data || []);
            } catch (e) {
                console.error("Lỗi tìm VĐV:", e);
            } finally {
                setIsP3Searching(false);
            }
        }, 400);
        return () => clearTimeout(delayDebounceFn);
    }, [p3SearchQuery]);
    ...
    const fetchScheduleMatches = useCallback(async (page: number, filters: { search: string, status: string }, append = false) => {
        ...
        try {
            ...
            if (json.success) {
                ...
                setScheduleHasMore((json.data.pagination?.page ?? page) < (json.data.pagination?.totalPages ?? 1));
            }
        } catch (e) {
    ...
    const handleRegisterClick = () => {
        ...
        if (myRegistration) {
            if (myRegistration.status === 'approved' || myRegistration.status === 'active') {
    ...
    const handleCancelRegistration = async () => {
        ...
        try {
            const res = await tournamentAPI.handleRegistration(id, { action: "cancel" });
            if (res.success) {
    ...
    const loadBrackets = async () => {
        try {
            // Fetch ALL matches for bracket display (no pagination limit)
            const res = await fetch(`/api/tournaments/${id}/matches?page=1&limit=500`);
            const json = await res.json();
            if (json.success) setBrackets(json.data);
        } catch (e) {
    ...
    const handleRegister = async (e: React.FormEvent) => {
        ...
        const isTeam2v2 = t.teamSize === 2;
        const isTeam3v3 = t.teamSize === 3;
        const isTeammateMode = isTeam2v2 || isTeam3v3;

        // Safety check: 2v2/3v3 require Player 2
        if (isTeammateMode && !selectedP2UserId) {
            toast.error('Vui lòng quay lại bước 2 và chọn VĐV 2.');
            return;
        }

        // Safety check: 3v3 requires Player 3
        if (isTeam3v3 && !selectedP3UserId) {
            toast.error('Vui lòng quay lại bước 2 và chọn VĐV 3.');
            return;
        }

        if (t.teamSize === 1) { // 1v1 Mode
            if (isMobile) {
        ...
        try {
            ...
            const res = await fetch(`/api/tournaments/${id}/register`, { method: "POST", body: JSON.stringify({
                ...regForm,
                player2Id: selectedP2UserId,
                player2UserId: selectedP2UserId,
                player3Id: selectedP3UserId,
                player3UserId: selectedP3UserId,
    ...
    if (isLoading) return <div className="flex items-center justify-center min-h-[60vh]"><Loader2 className="w-8 h-8 animate-spin text-efb-red" /></div>;
    ...
    const isGroupStage = t.format === 'group_stage';
    t.teamSize = t.gameMode === "1v1" ? 1 : t.gameMode === "2v2" ? 2 : t.gameMode === "3v3" ? 3 : t.gameMode === "6v6" ? 6 : 1;
    ...
    // Compute bracketPosition.y and nextMatch for each match if not already set
    bracketRounds.forEach((round, rIndex) => {
        round.matches.forEach((match: any, mIdx: number) => {
            if (!match.bracketPosition) {
                match.bracketPosition = { y: mIdx };
            }
            // Set nextMatch reference for connector lines (each pair feeds into next round)
            if (rIndex < bracketRounds.length - 1 && !match.nextMatch) {
                match.nextMatch = true;
            }
        });
    });
        ...
        <>
            <section className="relative pt-24 pb-14">
                ...
                <div className="absolute inset-0 overflow-hidden">
                    <Image 
                        src={t.banner || t.thumbnail || "/images/banner/banner_6v6.png"} 
                        alt="" 
                        fill 
                        className="object-cover opacity-60" 
                        priority 
                    />
                    <div className="absolute inset-0 bg-gradient-to-b from-[#7A1414]/75 via-[#A01B1B]/45 to-white" />
                </div>
                ...
                <div className="max-w-[1200px] mx-auto px-6 relative z-10">
                    ...
                    >
                        ...
                        <div className="flex gap-1.5 mb-3 flex-wrap">
                            ...
                            <Badge className="bg-red-50 text-efb-red border-red-100 text-[10px] rounded-full px-2.5 py-0.5">{formatLabels[t.format] || t.format}</Badge>
                            {t.gameMode && (
                                <Badge className="bg-amber-50 text-amber-700 border-amber-200 text-[10px] rounded-full px-2.5 py-0.5 font-bold uppercase">Giải {t.gameMode}</Badge>
                            )}
                        ...
                        <div className="flex flex-wrap gap-x-4 gap-y-1.5 text-[12px] text-gray-500 mb-4">
                            <span className="flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5 text-red-400" />{formatDate(t.schedule?.tournamentStart)} - {formatDate(t.schedule?.tournamentEnd)}</span>
                        ...
                        <div className="mb-4">
                            <div className="flex items-center justify-between mb-1.5">
                                ...
                                <span className="text-[11px] font-semibold text-efb-red">{t.currentTeams}/{t.maxTeams} đội</span>
                            </div>
                            <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                                <motion.div
                                    ...
                                    className={`h-full rounded-full ${(t.currentTeams / t.maxTeams) >= 0.8 ? "bg-gradient-to-r from-red-400 to-red-500" : "bg-gradient-to-r from-efb-red/60 to-efb-red"}`}
                                />
                            </div>
                        ...
                        <div className="grid grid-cols-3 gap-3 mb-4">
                            <div className="bg-red-50/60 rounded-xl px-3 py-2.5 text-center">
                                <div className="flex items-center justify-center gap-1.5 mb-0.5">
                                    <Users className="w-3.5 h-3.5 text-efb-red" />
                                    <span className="text-[10px] text-efb-red/70 font-medium uppercase tracking-wider">Đội</span>
                                </div>
                        ...
                        <div className="flex flex-wrap items-center gap-2 pt-3 border-t border-gray-100">
                            {t.status === "registration" ? (
                                checkingRegistration ? (
                                    ...
                                    <Button
                                        disabled
                                        className="bg-gray-100 text-gray-400 border border-gray-200 rounded-lg h-9 px-4 text-xs font-medium cursor-not-allowed"
                                    >
                                        <UserPlus className="w-3.5 h-3.5 mr-1.5 text-gray-300" /> Đã hết slot
                                    </Button>
                                ) : (
                                    <Button
                                        onClick={handleRegisterClick}
                                        className="bg-gradient-to-r from-efb-red to-efb-red-light hover:from-efb-red/90 hover:to-efb-red-light/90 text-white rounded-lg h-9 px-5 text-xs font-semibold shadow-sm shadow-red-500/15 hover:shadow-md transition-all"
                                    >
                    ...
                    <div className="sticky top-16 z-30 bg-white border-b border-gray-200 sm:hidden">
                        <div className="grid grid-cols-5 gap-0">
                            {tabs.map((tab) => {
                                ...
                                const shortLabel = tab.key === "players" ? "VĐV" : tab.key === "schedule" ? "Lịch đấu" : tab.key === "bracket" ? "Sơ đồ" : tab.key === "videos" ? "Video" : "Tổng quan";
                                return (
                                    <button
                                        key={tab.key}
                    ...
                    <div className="mt-6">
                                ...
                                <div className="lg:col-span-2 space-y-4">
                                    {t.description && (
                                        <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
                                            ...
                                            <div className="text-[13px] text-gray-600 whitespace-pre-line leading-relaxed break-words overflow-hidden" style={{ overflowWrap: 'anywhere' }} dangerouslySetInnerHTML={{ __html: t.description.replace(/(https?:\/\/[^\s<]+)/g, '<a href="$1" target="_blank" rel="noopener noreferrer" class="text-efb-red hover:underline break-all">$1</a>') }} />
                                        </div>
                                    )}
                                    ...
                                    {t.rules && (
                                        <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
                                            ...
                                            <div className="text-[13px] text-gray-600 whitespace-pre-line leading-relaxed break-words overflow-hidden" style={{ overflowWrap: 'anywhere' }} dangerouslySetInnerHTML={{ __html: t.rules.replace(/(https?:\/\/[^\s<]+)/g, '<a href="$1" target="_blank" rel="noopener noreferrer" class="text-efb-red hover:underline break-all">$1</a>') }} />
                                        </div>
                                    )}

                                    <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
                                        <h3 className="font-semibold text-[13px] text-gray-900 flex items-center gap-2 mb-3"><Gamepad2 className="w-4 h-4 text-efb-red" />Thông tin giải đấu</h3>
                                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                                            {[
                                                { label: "Hình thức", value: formatLabels[t.format] || t.format, icon: Trophy, color: "text-efb-red bg-red-50" },
                                                { label: "Platform", value: platformLabels[t.platform] || t.platform || "—", icon: Gamepad2, color: "text-efb-red bg-red-50" },
                                                { label: "Chế độ", value: t.gameMode ? `Giải ${t.gameMode}` : "—", icon: Users, color: "text-amber-600 bg-amber-50" },
                                    ...
                                    {/* Schedule Timeline */}
                                    <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
                                        <h3 className="font-semibold text-[13px] text-gray-900 flex items-center gap-2 mb-3"><Calendar className="w-4 h-4 text-efb-red" />Lịch trình</h3>
                                        <div className="space-y-3">
                                            {[
                                                { label: "Bắt đầu đăng ký", date: t.schedule?.registrationStart, icon: UserPlus, color: "bg-red-500" },
                                ...
                                <div className="space-y-4">
                                    ...
                                    {t.status === "registration" && !myRegistration && (
                                        <motion.div
                                            ...
                                            animate={{ opacity: 1, y: 0 }}
                                            className="bg-gradient-to-br from-efb-red via-efb-red-light to-efb-red-dark rounded-2xl p-6 text-white shadow-xl shadow-red-950/10 relative overflow-hidden"
                                        >
                                            ...
                                            <div className="relative z-10">
                                                ...
                                                <p className="text-white/60 text-xs mb-4">Tham gia thi đấu cùng cộng đồng 6v6 Vietnam</p>
                                                ...
                                                {t.currentTeams >= t.maxTeams ? (
                                                    <Button
                                                        disabled
                                                        className="w-full h-10 bg-white/40 text-white/80 font-semibold rounded-xl shadow-sm cursor-not-allowed text-sm"
                                                    >
                                                        <UserPlus className="w-5 h-5 mr-2 opacity-50" />
                                                        Đã đầy slot
                                                    </Button>
                                                ) : (
                                                    <Button
                                                        onClick={handleRegisterClick}
                                                        className="w-full h-10 bg-white text-efb-red hover:bg-white/90 font-semibold rounded-xl shadow-md transition-all group text-sm"
                                                    >
                                    ...
                                    {myRegistration && (
                                        <div className={`rounded-2xl p-5 border-2 ${myRegistration.status === 'approved' || myRegistration.status === 'active' ? 'bg-emerald-50 border-emerald-200' : 'bg-amber-50 border-amber-200'}`}>
                                            <div className="flex items-center gap-2 mb-2">
                                                {myRegistration.status === 'approved' || myRegistration.status === 'active'
                                                    ? <CheckCircle2 className="w-5 h-5 text-emerald-600" />
                                                    : <Clock className="w-5 h-5 text-amber-600" />
                                                }
                                                <h3 className={`font-semibold text-sm ${myRegistration.status === 'approved' || myRegistration.status === 'active' ? 'text-emerald-700' : 'text-amber-700'}`}>
                                                    {myRegistration.status === 'approved' || myRegistration.status === 'active' ? 'Đã được duyệt' : 'Đang chờ duyệt'}
                                                </h3>
                                            </div>
                                            <p className={`text-sm ${myRegistration.status === 'approved' || myRegistration.status === 'active' ? 'text-emerald-600' : 'text-amber-600'}`}>
                                                {myRegistration.status === 'approved' || myRegistration.status === 'active'
                                    ...
                                    {/* Contact */}
                                    <div className="bg-white border border-gray-100 rounded-xl p-5 shadow-sm">
                                        <h3 className="font-semibold text-[13px] text-gray-900 flex items-center gap-2 mb-3"><Users className="w-4 h-4 text-efb-red" />Thông tin</h3>
                        ...
                        {activeTab === "bracket" && (
                            <div>
                                ...
                                {isRoundRobin || isGroupStage ? (
                                    /* ==================== ROUND ROBIN / GROUP STAGE VIEW ==================== */
                                    <div className="bg-[#FDFDFD] rounded-2xl border border-gray-100 relative custom-scrollbar">
                                        <div className="p-4 sm:p-6 space-y-6">
                                            ...
                                            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                                                <div className="px-4 py-3 border-b border-gray-100 flex items-center gap-2.5">
                                                    ...
                                                    <div>
                                                        <h2 className="text-sm font-extrabold text-gray-900">Lịch thi đấu</h2>
                                                    </div>
                                                </div>
                                                <div className="px-4 py-2 border-b border-gray-100 flex items-center gap-2 overflow-x-auto custom-scrollbar">
                                                    {bracketRounds.map((round, rIdx) => {
                                                        const roundNum = round.matches[0]?.round || rIdx + 1;
                                                        const isActive = activeRound === roundNum;
                                                        return (
                                                            <button
                                                                key={rIdx}
                                                                onClick={() => setActiveRound(roundNum)}
                                                                className={`px-3 py-1.5 rounded-lg text-[11px] font-bold transition-all whitespace-nowrap flex-shrink-0 ${
                                                                    isActive ? 'bg-blue-600 text-white' : 'bg-gray-50 text-gray-600 hover:bg-gray-100'
                                                                }`}
                                                            >
                                                                {round.name}
                                                            </button>
                                                ...
                                                <div className="p-4">
                                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                                        {bracketRounds
                                                            .filter(r => (r.matches[0]?.round || 1) === activeRound)
                                                            .flatMap(r => r.matches)
                                                            .map((match: any) => {
                                                                ...
                                                                const homePlayer = match.homeTeam?.player1 || match.homeTeam?.captain?.name || match.p1?.name || match.homeTeam?.name || 'TBD';
                                                                const awayPlayer = match.awayTeam?.player1 || match.awayTeam?.captain?.name || match.p2?.name || match.awayTeam?.name || 'TBD';

                                                                return (
                                                                    <div
                                                                        key={match._id || match.id}
                                                                        onClick={() => setSelectedMatch(match)}
                                                                        className={`rounded-xl border cursor-pointer transition-all overflow-hidden hover:scale-[1.02] ${
                                                                            isCompleted ? 'border-gray-200 bg-white shadow-sm' :
                                                                            isLive ? 'border-red-200 bg-red-50/30 shadow-sm' :
                                                                            'border-dashed border-gray-200 bg-gray-50'
                                                                        }`}
                                                                    >
                                                                        <div className={`px-2.5 py-1.5 flex items-center justify-between text-[9px] font-bold ${
                                                                            isCompleted ? 'bg-emerald-50 text-emerald-600' :
                                                                            isLive ? 'bg-red-500 text-white' :
                                                                            'bg-gray-100 text-gray-500'
                                                                        }`}>
                                                                            <span>{isCompleted ? 'Kết thúc' : isLive ? 'LIVE' : 'Chưa đá'}</span>
                                                                            <span>Trận {match.matchNumber || ''}</span>
                                                                        </div>
                                                                        <div className="p-2.5">
                                                                            {/* Home */}
                                                                            <div className={`flex items-center justify-between py-1 ${homeWin ? 'text-gray-900' : 'text-gray-600'}`}>
                                                                                <div className="flex items-center gap-1.5 min-w-0">
                                                                                    {(match.homeTeam?.logo || match.homeTeam?.captain?.avatar) ? (
                                                                                        <img src={match.homeTeam.logo || match.homeTeam.captain?.avatar} alt="" className="w-5 h-5 rounded-md object-cover flex-shrink-0" />
                                                                                    ) : (
                                                                                        <div className="w-5 h-5 rounded-md bg-blue-100 flex items-center justify-center text-[9px] font-bold text-blue-600 flex-shrink-0">
                                                                                            {homePlayer.charAt(0).toUpperCase()}
                                                                                        </div>
                                                                                    )}
                                                                                    <span className={`text-[11px] truncate ${homeWin ? 'font-bold' : 'font-medium'}`}>{homePlayer}</span>
                                                                                </div>
                                                                                <span className={`text-sm tabular-nums ml-2 ${homeWin ? 'font-black text-emerald-600' : isDraw && isCompleted ? 'font-bold text-amber-500' : 'font-semibold text-gray-400'}`}>
                                                                                    {isCompleted || isLive ? match.homeScore : '-'}
                                                                                </span>
                                                                            </div>
                                                                            {/* Away */}
                                                                            <div className={`flex items-center justify-between py-1 ${awayWin ? 'text-gray-900' : 'text-gray-600'}`}>
                                                                                <div className="flex items-center gap-1.5 min-w-0">
                                                                                    {(match.awayTeam?.logo || match.awayTeam?.captain?.avatar) ? (
                                                                                        <img src={match.awayTeam.logo || match.awayTeam.captain?.avatar} alt="" className="w-5 h-5 rounded-md object-cover flex-shrink-0" />
                                                                                    ) : (
                                                                                        <div className="w-5 h-5 rounded-md bg-rose-100 flex items-center justify-center text-[9px] font-bold text-rose-600 flex-shrink-0">
                                                                                            {awayPlayer.charAt(0).toUpperCase()}
                                                                                        </div>
                                                                                    )}
                                                                                    <span className={`text-[11px] truncate ${awayWin ? 'font-bold' : 'font-medium'}`}>{awayPlayer}</span>
                                                                                </div>
                                                                                <span className={`text-sm tabular-nums ml-2 ${awayWin ? 'font-black text-emerald-600' : isDraw && isCompleted ? 'font-bold text-amber-500' : 'font-semibold text-gray-400'}`}>
                        ...
                        {activeTab === "videos" && (() => {
                            const allVideos: any[] = t.videos || [];
                            // Auto-detect type for older videos that don't have it
                            const typedVideos = allVideos.map((v: any) => ({
                                ...v,
                                type: v.type || (
                                    /youtube\.com\/shorts\//i.test(v.url) ? "shorts" :
                                    /tiktok\.com/i.test(v.url) || /vm\.tiktok\.com/i.test(v.url) ? "tiktok" :
                                    extractYouTubeId(v.url) ? "youtube" : "other"
                                )
                            }));

                            const landscapeVids = typedVideos.filter((v: any) => v.type === "youtube" || v.type === "other");
                            const verticalVids = typedVideos.filter((v: any) => v.type === "shorts" || v.type === "tiktok");

                            const extractTikTokIdLocal = (url: string) => {
                                const m = url.match(/\/video\/(\d+)/);
                                if (m) return m[1];
                                const em = url.match(/tiktok\.com\/embed\/v2\/(\d+)/);
                                if (em) return em[1];
                                return null;
                            };
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="flex items-center justify-between">
                                    <div>
                                        <h3 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
                                            <PlaySquare className="w-5 h-5 text-efb-red" />
                                            Video Highlights
                                        </h3>
                                        <p className="text-xs text-gray-400 mt-0.5">{typedVideos.length} video</p>
                                    </div>
                                </div>
                                ...
                                {typedVideos.length === 0 ? (
                                    <div className="text-center py-20 bg-white rounded-2xl border border-gray-100">
                                        ...
                                        <p className="text-gray-400 text-xs mt-1">BTC chưa đăng video highlight cho giải đấu này</p>
                                    </div>
                                ) : (
                                    <>
                                        {/* YouTube / Other — Landscape 16:9 Grid */}
                                            ...
                                            <div className="space-y-3">
                                                {verticalVids.length > 0 && (
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <span className="text-sm font-bold text-gray-700">YouTube Videos</span>
                                                        <span className="text-[10px] text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full font-medium">{landscapeVids.length}</span>
                                                    </div>
                                                )}
                                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                                    {landscapeVids.map((vid: any, idx: number) => {
                                                        const ytId = extractYouTubeId(vid.url);
                                                        const isIframe = vid.url.trim().startsWith("<iframe");
                                                        const iframeSrc = isIframe ? extractIframeSrc(vid.url) : null;
                                                        return (
                                                            <motion.div key={`yt-${idx}`} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }}
                                                                ...
                                                                <div className="aspect-video bg-black relative">
                                                                    {ytId ? (
                                                                        <iframe src={getYouTubeEmbedUrl(ytId)} className="w-full h-full" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen title={vid.title} />
                                                                    ) : isIframe && iframeSrc ? (
                                                                        <iframe src={iframeSrc} className="w-full h-full" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen title={vid.title} />
                                                                    ) : /\.(mp4|webm|ogg)(\?|$)/i.test(vid.url) ? (
                                                                        <video src={vid.url} controls className="w-full h-full object-contain" preload="metadata" />
                                                                    ) : (
                                                                        <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-gray-800 to-gray-900">
                                                                            <a href={vid.url} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center gap-2 text-gray-400 hover:text-white transition-colors">
                                                                                <ExternalLink className="w-6 h-6" /><span className="text-xs">Mở link</span>
                                                                            </a>
                                                                        </div>
                                                                    )}
                                                                    {ytId && (
                                                                        <div className="absolute top-2 left-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                                            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-red-600/90 text-white text-[9px] font-bold backdrop-blur-sm">
                                                                ...
                                                                <div className="p-4">
                                                                    <h4 className="text-sm font-bold text-gray-900 line-clamp-2 leading-snug">{vid.title}</h4>
                                                                    {vid.createdAt && (
                                                                        <p className="text-[10px] text-gray-400 mt-2 flex items-center gap-1">
                                                                            <Clock className="w-3 h-3" />
                                                                            {new Date(vid.createdAt).toLocaleDateString("vi-VN", { day: "2-digit", month: "2-digit", year: "numeric" })}
                                                                        </p>
                                                                    )}
                                                                </div>
                                                            </motion.div>
                                        ...
                                        {/* Shorts + TikTok — Vertical 9:16 Scroll */}
                                        {verticalVids.length > 0 && (
                                            <div className="space-y-3">
                                                <div className="flex items-center gap-2">
                                                    <svg className="w-4 h-4 text-pink-500" viewBox="0 0 24 24" fill="currentColor"><path d="M10 15l5.19-3L10 9v6m11.56-7.83c.13.47.22 1.1.28 1.9.07.8.1 1.49.1 2.09L22 12c0 2.19-.16 3.8-.44 4.83-.25.9-.83 1.48-1.73 1.73-.47.13-1.33.22-2.65.28-1.3.07-2.49.1-3.59.1L12 19c-4.19 0-6.8-.16-7.83-.44-.9-.25-1.48-.83-1.73-1.73-.13-.47-.22-1.1-.28-1.9-.07-.8-.1-1.49-.1-2.09L2 12c0-2.19.16-3.8.44-4.83.25-.9.83-1.48 1.73-1.73.47-.13 1.33-.22 2.65-.28 1.3-.07 2.49-.1 3.59-.1L12 5c4.19 0 6.8.16 7.83.44.9.25 1.48.83 1.73 1.73z"/></svg>
                                                    <span className="text-sm font-bold text-gray-700">Shorts & TikTok</span>
                                                    <span className="text-[10px] text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full font-medium">{verticalVids.length}</span>
                                                </div>
                                                {/* Horizontal scroll on mobile, grid on desktop */}
                                                <div className="flex gap-4 overflow-x-auto pb-4 snap-x snap-mandatory scrollbar-hide sm:grid sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 sm:overflow-visible sm:pb-0">
                                                    {verticalVids.map((vid: any, idx: number) => {
                                                        const ytId = extractYouTubeId(vid.url);
                                                        const tiktokId = extractTikTokIdLocal(vid.url);
                                                        const isShorts = vid.type === "shorts";
                                                        const isTiktok = vid.type === "tiktok";

                                                        return (
                                                            <motion.div key={`vert-${idx}`} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: idx * 0.05 }}
                                                                className="flex-shrink-0 w-[200px] sm:w-auto snap-center group bg-white rounded-xl border border-gray-100 overflow-hidden hover:shadow-lg hover:border-gray-200 transition-all duration-300">
                                                                {/* Video embed */}
                                                                <div className="relative aspect-[9/16] bg-black overflow-hidden">
                                                                    {isShorts && ytId ? (
                                                                        <iframe src={`https://www.youtube.com/embed/${ytId}`} className="w-full h-full" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen title={vid.title} />
                                                                    ) : isTiktok && tiktokId ? (
                                                                        <iframe src={`https://www.tiktok.com/embed/v2/${tiktokId}?lang=vi`} className="w-full h-full" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen title={vid.title} />
                                                                    ) : (
                                                                        /* Fallback: show thumbnail + link */
                                                                        <a href={vid.url} target="_blank" rel="noopener noreferrer" className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-gray-800 to-gray-900 text-gray-400 hover:text-white transition-colors group/link">
                                                                            {ytId ? (
                                                                                <img src={getYouTubeThumbnail(ytId)} alt={vid.title} className="w-full h-full object-cover opacity-60" />
                                                                            ) : (
                                                                                <>
                                                                                    <ExternalLink className="w-8 h-8 mb-2" />
                                                                                    <span className="text-xs">Mở video</span>
                                                                                </>
                                                                            )}
                                                                        </a>
                                                                    )}
                                                                    ...
                                                                    <div className="absolute top-2 left-2 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                                                                        {isShorts && (
                                                                            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-pink-600/90 text-white text-[9px] font-bold backdrop-blur-sm">
                                                                                <PlaySquare className="w-3 h-3" /> Shorts
                                                                            </span>
                                                                        )}
                                                                        {isTiktok && (
                                                                            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-gray-900/90 text-white text-[9px] font-bold backdrop-blur-sm">
                                                                                <svg className="w-3 h-3" viewBox="0 0 24 24" fill="currentColor"><path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-2.88 2.5 2.89 2.89 0 0 1 0-5.78c.27 0 .54.04.8.1v-3.5a6.37 6.37 0 0 0-.8-.05A6.34 6.34 0 0 0 3.15 15.2 6.34 6.34 0 0 0 9.49 21.6a6.34 6.34 0 0 0 6.34-6.34V8.7a8.16 8.16 0 0 0 3.76.92V6.18a4.83 4.83 0 0 1-.01.51z"/></svg>
                                                                                TikTok
                                                                            </span>
                                                                        )}
                                                                    </div>
                                                                ...
                                                                {/* Title */}
                                                                <div className="p-3">
                                                                    <h4 className="text-xs font-bold text-gray-900 line-clamp-2 leading-snug">{vid.title}</h4>
                                                                    {vid.createdAt && (
                            ...
                            <div className="space-y-4">
                                        ...
                                        {t.efvTier && (
                                            <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider ${t.efvTier === 'efv_1000' ? 'bg-amber-100 text-amber-700 border border-amber-200' :
                                                t.efvTier === 'efv_500' ? 'bg-purple-100 text-purple-700 border border-purple-200' :
                                                    'bg-red-100 text-efb-red-light border border-red-200'
                                                }`}>
                                ...
                                <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
                                    ...
                                    {!playerLoading && (() => {
                                        ...
                                        const placementColorMap: Record<string, string> = { champion: 'bg-amber-100 text-amber-700 border border-amber-200', runner_up: 'bg-slate-100 text-slate-700 border border-slate-200', top_4: 'bg-orange-50 text-orange-600 border border-orange-200', top_8: 'bg-red-50 text-efb-red border border-red-200', top_16: 'bg-red-50 text-efb-red border border-red-200', top_32: 'bg-gray-50 text-gray-500' };
                                        ...
                                        <div className="overflow-x-auto">
                                            <table className="w-full text-sm" style={{ minWidth: '600px' }}>
                                                <thead>
                                                    <tr className="bg-gradient-to-r from-slate-800 to-slate-900 text-white text-[10px] uppercase tracking-wider">
                                                        ...
                                                        {isElimFormat ? (
                                                            ...
                                                            <>
                                                                <th className="px-2 sm:px-3 py-3 text-center w-8 sm:w-10">P</th>
                                                                <th className="px-2 sm:px-3 py-3 text-center w-8 sm:w-10">W</th>
                                                                <th className="px-2 sm:px-3 py-3 text-center w-8 sm:w-10">D</th>
                                                                <th className="px-2 sm:px-3 py-3 text-center w-8 sm:w-10">L</th>
                                                                <th className="px-2 sm:px-3 py-3 text-center w-8 sm:w-10">BT</th>
                                                                <th className="px-2 sm:px-3 py-3 text-center w-8 sm:w-10">BB</th>
                                                                <th className="px-2 sm:px-3 py-3 text-center w-10 sm:w-12">HS</th>
                                                                <th className="px-2 sm:px-3 py-3 text-center w-20 sm:w-24">Phong độ</th>
                                                                <th className="px-3 sm:px-4 py-3 text-center w-12 sm:w-16">Điểm</th>
                                                            </>
                                                        )}
                                                ...
                                                <tbody>
                                                    {(() => {
                                                        const isRoundRobinOrGroup = t.format === 'round_robin' || t.format === 'group_stage';
                                                        let displayTeams = isRoundRobinOrGroup ? [...teams].sort((a, b) => {
                                                            const sA = a.stats || {};
                                                            const sB = b.stats || {};
                                                            const pA = sA.points || 0;
                                                            const pB = sB.points || 0;
                                                            if (pA !== pB) return pB - pA;
                                                            const gdA = sA.goalDifference || 0;
                                                            const gdB = sB.goalDifference || 0;
                                                            if (gdA !== gdB) return gdB - gdA;
                                                            return (sB.goalsFor || 0) - (sA.goalsFor || 0);
                                                        }) : (playerData?.teams || []);

                                                        if (isRoundRobinOrGroup && playerSearch.trim()) {
                                                            displayTeams = displayTeams.filter(team => {
                                                                const reg = team._reg || {};
                                                                return [
                                                                    team.name, team.shortName, team.captain?.name, reg.playerName, reg.player2Name, reg.player3Name, team.player1, team.player2, team.player3
                                                                ].some(v => v && v.toLowerCase().includes(playerSearch.toLowerCase()));
                                                            });
                                                        }

                                                        return displayTeams.map((team: any, pageIdx: number) => {
                                                        const pg = playerData?.pagination;
                                                        const i = isRoundRobinOrGroup ? pageIdx : ((pg?.page || 1) - 1) * (pg?.limit || 100) + pageIdx;
                                                        const reg = team._reg || {};
                                                        ...
                                                        const avatarSrc = t.gameMode === '6v6'
                                                            ? (team.logo || reg?.personalPhoto || team.captain?.avatar || null)
                                                            : (team.captain?.avatar || reg?.user?.avatar || reg?.personalPhoto || null);
                                                        ...
                                                        return (
                                                            <tr key={team._id} className={`border-b border-gray-50 last:border-0 transition-colors hover:bg-red-50/30 ${isTop1 ? 'bg-amber-50/40' : isTop2 ? 'bg-gray-50/40' : ''}`}
                                                            >
                                                                ...
                                                                <td className="px-3 sm:px-4 py-3">
                                                                    <div className="flex items-center gap-2.5 sm:gap-3">
                                                                        ...
                                                                        <div className="min-w-0 flex flex-col gap-[2px]">
                                                                            ...
                                                                            {(() => {
                                                                                const membersExceptCaptain = (team.members || []).filter((m: any) => m?.user && m?.user?._id !== team?.captain?._id);
                                                                                const t2Name = reg.player2Name || membersExceptCaptain[0]?.user?.name;
                                                                                const t3Name = reg.player3Name || membersExceptCaptain[1]?.user?.name;
                                                                                return (
                                                                                    <>
                                                                                        {(t.teamSize === 2 || t.teamSize === 3) && t2Name && (
                                                                                            <p className="text-[11px] sm:text-[12px] font-bold text-gray-500 truncate tracking-tight">/ {t2Name}</p>
                                                                                        )}
                                                                                        {t.teamSize === 3 && t3Name && (
                                                                                            <p className="text-[11px] sm:text-[12px] font-bold text-gray-500 truncate tracking-tight">/ {t3Name}</p>
                                                                                        )}
                                                                                    </>
                                                                                );
                                                                            })()}
                                                                ...
                                                                {isElimFormat ? (
                                                                    ...
                                                                    const bxhTeamStats = teams.find((tBXH: any) => tBXH._id === team._id)?.stats || team.stats || {};
                                                                    const gd = bxhTeamStats.goalDifference || 0;
                                                                    return (
                                                                    <>
                                                                        <td className="px-2 sm:px-3 py-3 text-center text-[11px] sm:text-[13px] font-medium text-gray-600">{bxhTeamStats.played || 0}</td>
                                                                        <td className="px-2 sm:px-3 py-3 text-center text-[11px] sm:text-[13px] font-bold text-emerald-600">{bxhTeamStats.wins || 0}</td>
                                                                        <td className="px-2 sm:px-3 py-3 text-center text-[11px] sm:text-[13px] font-medium text-gray-500">{bxhTeamStats.draws || 0}</td>
                                                                        <td className="px-2 sm:px-3 py-3 text-center text-[11px] sm:text-[13px] font-medium text-rose-500">{bxhTeamStats.losses || 0}</td>
                                                                        <td className="px-2 sm:px-3 py-3 text-center text-[11px] sm:text-[13px] font-medium text-gray-400">{bxhTeamStats.goalsFor || 0}</td>
                                                                        <td className="px-2 sm:px-3 py-3 text-center text-[11px] sm:text-[13px] font-medium text-gray-400">{bxhTeamStats.goalsAgainst || 0}</td>
                                                                        <td className="px-2 sm:px-3 py-3 text-center text-[11px] sm:text-[13px] font-semibold text-gray-700">{gd > 0 ? `+${gd}` : gd}</td>
                                                                        <td className="px-2 py-3">
                                                                            <div className="flex items-center justify-center gap-[2px]">
                                                                                {(bxhTeamStats.form || []).slice(-5).map((f: string, fi: number) => (
                                                                                    <span key={fi} className={`w-3 h-3 sm:w-4 sm:h-4 rounded-full text-[7px] sm:text-[9px] font-bold flex items-center justify-center ${
                                                                                        f === 'W' ? 'bg-emerald-500 text-white' :
                                                                                        f === 'D' ? 'bg-gray-300 text-gray-700' :
                                                                        ...
                                                                        <td className="px-3 sm:px-4 py-3 text-center">
                                                                            <span className="inline-flex items-center justify-center min-w-[28px] sm:min-w-[32px] h-6 sm:h-7 rounded-lg bg-red-50 text-efb-red font-bold text-[11px] sm:text-xs px-1.5 sm:px-2">
                                                                                {bxhTeamStats.points || 0}
                                                                            </span>
                                                                        </td>
                                                                    </>
                                                                    );
                                                                ...
                                                                <td className="px-2 sm:px-3 py-3 text-center">
                                                                    {reg?.teamLineupPhoto ? (
                                                                        <button
                                                                            ...
                                                                            className="inline-flex items-center justify-center w-7 h-7 rounded-lg bg-red-50 hover:bg-red-100 text-efb-red hover:text-efb-red transition-all"
                                                                            title="Xem đội hình"
                                                                        >
                                    ...
                                    {!playerLoading && ((isRoundRobin || isGroupStage) ? teams.length === 0 : (!playerData?.teams || playerData.teams.length === 0)) && (
                                    ...
                                    {/* Pagination */}
                                    {!(isRoundRobin || isGroupStage) && playerData && playerData.pagination.totalPages > 1 && (
                        ...
                        {activeTab === "schedule" && (() => {
                            ...
                            return (
                                <div className="space-y-0">
                                    ...
                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
                                        <div>
                                            <h3 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
                                                <Calendar className="w-5 h-5 text-efb-red" /> Lịch thi đấu
                                            </h3>
                                        ...
                                        <div className="relative">
                                            <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
                                            <Input placeholder="Tìm VĐV, đội..." value={bracketSearch} onChange={(e) => { setBracketSearch(e.target.value); /* reset handled by effect */ }} className="pl-9 h-9 text-sm rounded-lg border-gray-200 w-[220px] focus:border-efb-red" />
                                    ...
                                    <div className="flex gap-1.5 mb-4 flex-wrap">
                                        {([
                                            ...
                                            <button key={f.key} onClick={() => { setScheduleFilter(f.key); /* reset handled by effect */ }} className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${scheduleFilter === f.key ? (f.key === 'live' ? 'bg-red-500 text-white shadow-sm' : 'bg-efb-red text-white shadow-sm') : 'bg-gray-50 text-gray-500 hover:bg-gray-100 border border-gray-100'}`}>
                                    ...
                                    {/* Info bar */}
                                    <div className="flex items-center justify-between bg-red-50/60 px-4 py-2.5 rounded-xl border border-red-100 mb-4">
                                        <div className="flex items-center gap-4 text-xs text-gray-600">
                                            ...
                                            <span className="hidden sm:inline text-efb-red font-semibold">{completedCount}/{totalCount} trận hoàn thành</span>
                                        </div>
                                        <div className="flex items-center gap-3 text-[10px]">
                                            ...
                                            <span className="flex items-center gap-1 text-gray-400"><span className="w-2 h-2 rounded-full bg-amber-400" /> Chờ</span>
                                        </div>
                                    </div>
                                                        ...
                                                        <div key={roundName}>
                                                            ...
                                                            <div className="divide-y divide-gray-50">
                                                                {filtered.map((m: any) => {
                                                                    ...
                                                                    return (
                                                                        <div key={m._id} className={`grid grid-cols-12 gap-4 items-center py-2.5 px-4 hover:bg-red-50/30 transition-colors cursor-pointer ${isLive ? 'bg-red-50/30' : ''}`} onClick={() => setSelectedMatch(m)}>
                                                                            ...
                                                                            <div className="col-span-2 flex flex-col gap-1.5">
                                                                                <div className="border border-gray-200 rounded px-1.5 py-0.5 shadow-sm bg-white text-[11px] font-semibold text-gray-700 truncate w-fit max-w-full flex items-center gap-1">
                                                                                    <div className="w-1.5 h-1.5 rounded-full bg-efb-red flex-shrink-0" />
                                                                                    
                                                                                    <span className="truncate">{homeName}</span>
                                                                                </div>
                                                                                <div className="border border-gray-200 rounded px-1.5 py-0.5 shadow-sm bg-white text-[11px] font-semibold text-gray-700 truncate w-fit max-w-full flex items-center gap-1">
                                                                                    <div className="w-1.5 h-1.5 rounded-full bg-efb-gold flex-shrink-0" />
                                                                            ...
                                                                            <div className="col-span-2 flex items-center justify-center">
                                                                                <div className={`px-2.5 py-1 rounded-md text-[11px] font-semibold ${isCompleted ? "bg-emerald-50 text-emerald-600 border border-emerald-100" : isLive ? "bg-red-50 text-red-600 border border-red-100 animate-pulse" : "bg-red-50/50 border border-red-100 text-red-400"}`}>
                                                            ...
                                                            <div className={`flex items-center gap-2 min-w-0 flex-1 ${isRight ? 'flex-row-reverse text-right' : ''} ${isWin ? '' : isCompleted ? 'opacity-50' : ''}`}>
                                                                ...
                                                                <div className="min-w-0 flex-1">
                                                                    <div className={`flex items-center gap-1 flex-wrap ${isRight ? 'justify-end' : ''}`}>
                                                                        ...
                                                                        <span className={`text-[12px] font-bold ${isWin ? 'text-efb-red-light' : 'text-gray-800'}`} style={{ wordBreak: 'break-word' }}>{name}</span>
                                                                    </div>
                                                    ...
                                                    return (
                                                        <div key={m._id} className={`px-3 py-3 cursor-pointer active:bg-gray-50 transition-colors ${isLive ? 'bg-red-50/30' : ''}`} onClick={() => setSelectedMatch(m)}>
                                                            <div className="flex items-center justify-between mb-2">
                                                                ...
                                                                <div className={`px-2 py-0.5 rounded text-[10px] font-semibold ${isCompleted ? "bg-emerald-50 text-emerald-600" : isLive ? "bg-red-50 text-red-600 animate-pulse" : "bg-red-50/50 text-red-400"}`}>
                            ...
                            {scheduleHasMore && (
                                <div ref={scheduleObserverTarget} className="mt-6 flex flex-col items-center justify-center pb-6 gap-2">
                                    <Loader2 className="w-6 h-6 text-efb-red animate-spin" />
                                    <p className="text-sm text-gray-400">Đang tải thêm trận đấu...</p>
                                            ...
                                            {totalFiltered === 0 && (
                                                <div className="text-center py-12">
                                                    ...
                                                    {bracketSearch && <button onClick={() => { setBracketSearch(""); /* reset handled by effect */ }} className="text-xs text-efb-red hover:underline mt-1">Xóa tìm kiếm</button>}
                                                </div>
                                            )}
                                        </>)}
                                    </div>
                    ...
                    {selectedPlayer && (() => {
                        ...
                        return (
                            <>
                                ...
                                <div className="bg-gradient-to-br from-efb-red via-efb-red-dark to-red-950 px-6 pt-6 pb-5 text-white relative overflow-hidden">
                                ...
                                <div className="px-6 py-5 space-y-4">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div className="space-y-2 text-[13px]">
                                            {(reg?.facebookName || reg?.facebookLink) && (
                                                <div className="flex justify-between items-center">
                                                    <span className="text-gray-400 flex items-center gap-1.5"><Globe className="w-3.5 h-3.5" />Facebook</span>
                                                    {reg.facebookLink ? (
                                                        <a href={reg.facebookLink} target="_blank" className="font-semibold text-efb-red hover:underline text-right truncate max-w-[180px]">{reg.facebookName || 'Xem'}</a>
                                                    ) : (
                                    ...
                                    {team.stats?.points !== undefined && (
                                        <div className="bg-red-50 rounded-xl p-3 flex items-center justify-between">
                                            <span className="text-xs text-efb-red font-medium">Tổng điểm</span>
                                            <span className="text-lg font-bold text-efb-red-light">{team.stats.points}</span>
                                        </div>
                                    )}
                                ...
                                {[{ step: 1, label: "Cá nhân" }, { step: 2, label: "Game" }, { step: 3, label: "Xác nhận" }].map((s, i) => (
                                    <div key={s.step} className="flex items-center flex-1">
                                        <button type="button" onClick={() => { if (s.step < regStep) setRegStep(s.step); }} className="flex items-center gap-2.5 group">
                                            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-semibold transition-all duration-300 ${regStep === s.step ? 'bg-efb-red text-white ring-4 ring-red-100 scale-110' :
                                                regStep > s.step ? 'bg-efb-red text-white' :
                                                    'bg-gray-100 text-gray-400'
                                                }`}>
                                                {regStep > s.step ? <CheckCircle2 className="w-3.5 h-3.5" /> : s.step}
                                            </div>
                                            <span className={`text-xs font-medium transition-colors hidden sm:inline ${regStep === s.step ? 'text-efb-red' :
                                        ...
                                        {i < 2 && (
                                            <div className="flex-1 mx-3 h-0.5 rounded-full bg-gray-100 overflow-hidden">
                                                <div className={`h-full rounded-full transition-all duration-500 ease-out ${regStep > s.step ? 'w-full bg-efb-red' : 'w-0 bg-transparent'}`} />
                                            </div>
                                        )}
                                    </div>
                                ))}
                        ...
                        <AnimatePresence mode="wait">
                            {regStep === 1 && (
                                <motion.div key="s1" initial={{ opacity: 0, x: 15 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -15 }} className="space-y-4">
                                    <div className={`grid grid-cols-1 ${t.teamSize >= 2 ? 'gap-4' : 'sm:grid-cols-2 gap-4'}`}>
                                        <div className="space-y-1.5"><Label className="text-xs font-medium text-gray-500">Họ và tên VĐV 1 <span className="text-red-400">*</span></Label><Input placeholder="Nguyễn Văn A" value={regForm.playerName} onChange={e => setRegForm({ ...regForm, playerName: e.target.value })} required className="h-11 rounded-lg border-gray-200 focus:border-efb-red bg-gray-50/50 focus:bg-white transition-all text-sm" /></div>
                                            ...
                                            <div className="space-y-1.5">
                                                ...
                                                <Popover open={countryOpen} onOpenChange={(open) => { setCountryOpen(open); if (open && countries.length === 0) { fetch('https://restcountries.com/v3.1/all?fields=name,cca2').then(r => r.json()).then((data: { name: { common: string }; cca2: string }[]) => { const sorted = data.map(c => ({ name: c.name.common, code: c.cca2 })).sort((a, b) => { if (a.name === 'Vietnam') return -1; if (b.name === 'Vietnam') return 1; return a.name.localeCompare(b.name); }); const vnIdx = sorted.findIndex(c => c.code === 'VN'); if (vnIdx > 0) { const vn = sorted.splice(vnIdx, 1)[0]; vn.name = 'Việt Nam'; sorted.unshift(vn); } setCountries(sorted); }).catch(() => { setCountries([{ name: 'Việt Nam', code: 'VN' }, { name: 'Japan', code: 'JP' }, { name: 'South Korea', code: 'KR' }, { name: 'Thailand', code: 'TH' }]); }); } }}>
                                                    <PopoverTrigger asChild>
                                                        <button type="button" className={`flex h-11 w-full items-center justify-between rounded-lg border border-gray-200 bg-gray-50/50 px-3 text-sm transition-all hover:bg-white focus:outline-none focus:border-efb-red focus:bg-white ${!regCountry ? 'text-gray-400' : 'text-gray-900'}`}>
                                            ...
                                            <div className="space-y-1.5">
                                                ...
                                                {regCountry === 'Việt Nam' ? (
                                                    <Popover open={provinceOpen} onOpenChange={(open) => { setProvinceOpen(open); if (open && vnProvinces.length === 0) { fetch('https://provinces.open-api.vn/api/p/').then(r => r.json()).then((data: { name: string; code: number }[]) => setVnProvinces(data)).catch(() => { }); } }}>
                                                        <PopoverTrigger asChild>
                                                            <button type="button" className={`flex h-11 w-full items-center justify-between rounded-lg border border-gray-200 bg-gray-50/50 px-3 text-sm transition-all hover:bg-white focus:outline-none focus:border-efb-red focus:bg-white ${!regForm.province ? 'text-gray-400' : 'text-gray-900'}`}>
                                                                <div className="flex items-center gap-2 truncate">
                                                    ...
                                                    <div className="relative">
                                                        ...
                                                        <Input placeholder="Nhập tên thành phố..." value={regForm.province} onChange={e => setRegForm({ ...regForm, province: e.target.value })} className="h-11 pl-10 rounded-lg border-gray-200 focus:border-efb-red bg-gray-50/50 focus:bg-white transition-all text-sm" />
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                                    )}
                                    <div className="pt-3 flex justify-end"><Button type="button" onClick={() => { if (!regForm.playerName.trim() || !regForm.phone.trim() || (t.teamSize < 2 && !regForm.dateOfBirth)) { toast.error('Vui lòng nhập đầy đủ thông tin bắt buộc'); return; } setRegStep(2); }} className="h-11 px-8 bg-efb-red text-white rounded-lg font-medium flex items-center gap-2">Tiếp theo <ChevronRight className="w-4 h-4" /></Button></div>
                                </motion.div>
                            )}
                            {regStep === 2 && (
                                <motion.div key="s2" initial={{ opacity: 0, x: 15 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -15 }} className="space-y-4">
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        <div className="space-y-1.5"><Label className="text-xs font-medium text-gray-500">{t.teamSize >= 2 ? "Tên Facebook VĐV 1" : "Facebook"} <span className="text-red-400">*</span></Label><div className="relative"><Globe className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" /><Input placeholder="Tên Facebook" value={regForm.facebookName} onChange={e => setRegForm({ ...regForm, facebookName: e.target.value })} required className="h-11 pl-10 rounded-lg border-gray-200 focus:border-efb-red bg-gray-50/50 focus:bg-white transition-all text-sm" /></div></div>
                                        <div className="space-y-1.5"><Label className="text-xs font-medium text-gray-500">{t.teamSize >= 2 ? "Link Facebook VĐV 1" : "Link Facebook"} <span className="text-red-400">*</span></Label><div className="relative"><ExternalLink className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" /><Input placeholder="https://facebook.com/..." value={regForm.facebookLink} onChange={e => setRegForm({ ...regForm, facebookLink: e.target.value })} required className="h-11 pl-10 rounded-lg border-gray-200 focus:border-efb-red bg-gray-50/50 focus:bg-white transition-all text-sm" /></div></div>
                                    </div>
                                    
                                    {(t.teamSize === 2 || t.teamSize === 3) && (
                                        <div className="pt-3 pb-1 border-t border-gray-100 border-dashed space-y-4">
                                            <div className="flex items-center justify-between">
                                                ...
                                                {selectedP2UserId ? (
                                                    <span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2 py-1 rounded-md border border-emerald-200 tracking-wider flex items-center gap-1"><CheckCircle2 className="w-3 h-3" />Đã Liên Kết</span>
                                                ) : (
                                            ...
                                            <div className="grid grid-cols-1 gap-4">
                                                <div className="space-y-1.5 relative">
                                                    <Label className="text-xs font-medium text-gray-500">Tìm VĐV 2 trong hệ thống <span className="text-[10px] font-normal text-amber-500">(VĐV 2 bắt buộc có tài khoản)</span> <span className="text-red-400">*</span></Label>
                                                    <div className="relative">
                                                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                                                        <Input 
                                                            placeholder="Tìm bằng Tên hoặc Nickname..." 
                                                            value={showP2Dropdown ? p2SearchQuery : regForm.player2Name} 
                                                            onFocus={() => { if (!selectedP2UserId) { setShowP2Dropdown(true); setP2SearchQuery(regForm.player2Name); } }}
                                                            onChange={e => {
                                                                if (selectedP2UserId) return;
                                                                if (!showP2Dropdown) setShowP2Dropdown(true);
                                                                setP2SearchQuery(e.target.value);
                                                                setRegForm({ ...regForm, player2Name: e.target.value });
                                                            }} 
                                                            readOnly={!!selectedP2UserId}
                                                            required 
                                                            className={`h-11 pl-10 rounded-lg border-gray-200 focus:border-efb-red focus:bg-white transition-all text-sm ${selectedP2UserId ? 'bg-emerald-50/40 border-emerald-300 cursor-default' : 'bg-gray-50/50'}`} 
                                                        />
                                                        {isP2Searching && <Loader2 className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-emerald-500 animate-spin" />}
                                                        {selectedP2UserId && (
                                                            <button type="button" onClick={() => { setSelectedP2UserId(null); setRegForm({ ...regForm, player2Name: '', player2FacebookName: '', player2FacebookLink: '' }); setP2SearchQuery(''); setShowP2Dropdown(true); }} className="absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 rounded-full bg-red-100 hover:bg-red-200 flex items-center justify-center transition-colors" title="Xóa liên kết VĐV 2">
                                                        ...
                                                        <div className="absolute z-50 mt-1 w-full bg-white rounded-xl shadow-xl border border-gray-100 max-h-60 overflow-y-auto overflow-x-hidden">
                                                            {p2SearchResults.length > 0 ? (
                                                                    ...
                                                                    {p2SearchResults.map((u: any) => (
                                                                        <button 
                                                                            ...
                                                                            onClick={() => {
                                                                                ...
                                                                                setSelectedP2UserId(u._id);
                                                                                setP2SearchQuery(u.name);
                                                                                setShowP2Dropdown(false);
                                                                            }}
                                                                            className={`w-full text-left px-3 py-2.5 rounded-lg transition-colors flex items-center gap-3 ${u.isRegistered ? 'opacity-50 cursor-not-allowed bg-red-50/30' : 'hover:bg-gray-50'}`}
                                                                        >
                                                                            ...
                                                                            <div className="flex-1 min-w-0">
                                                                                <div className="flex items-center gap-1.5">
                                                                                    ...
                                                                                    {u.playerId && <span className="text-[10px] font-bold text-blue-600 bg-blue-50 border border-blue-200 px-1.5 py-px rounded flex-shrink-0">#{u.playerId}</span>}
                                                                                </div>
                                                                                <div className="flex items-center gap-2 mt-0.5">
                                                                                    {u.nickname && <span className="text-[10px] text-gray-500 truncate">{u.nickname}</span>}
                                                                                    {u.isRegistered && <span className="text-[9px] font-bold text-red-500 bg-red-50 border border-red-200 px-1.5 py-px rounded-full flex-shrink-0">Đã tham gia {u.registeredTeam || 'đội khác'}</span>}
                                                                                </div>
                                                                            </div>
                                                                        </button>
                                                                    ))}
                                                                ...
                                                                <div className="p-4 text-center">
                                                                    ...
                                                                    <p className="text-[11px] text-gray-400 mt-1">VĐV 2 phải có tài khoản trên hệ thống.<br/>Vui lòng yêu cầu VĐV 2 đăng ký tài khoản trước.</p>
                                            ...
                                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                                <div className="space-y-1.5"><Label className="text-xs font-medium text-gray-500">Tên Facebook VĐV 2 <span className="text-red-400">*</span> {selectedP2UserId && <span className="text-[10px] text-emerald-500 font-normal">(Tự động điền)</span>}</Label><div className="relative"><Globe className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" /><Input placeholder="Tên Facebook" value={regForm.player2FacebookName} onChange={e => setRegForm({ ...regForm, player2FacebookName: e.target.value })} onFocus={() => { setShowP2Dropdown(false); setShowP3Dropdown(false); }} required className={`h-11 pl-10 rounded-lg border-gray-200 focus:border-efb-red focus:bg-white transition-all text-sm ${selectedP2UserId ? 'bg-emerald-50/30' : 'bg-gray-50/50'}`} /></div></div>
                                                <div className="space-y-1.5"><Label className="text-xs font-medium text-gray-500">Link Facebook VĐV 2 <span className="text-red-400">*</span> {selectedP2UserId && <span className="text-[10px] text-emerald-500 font-normal">(Tự động điền)</span>}</Label><div className="relative"><ExternalLink className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" /><Input placeholder="https://facebook.com/..." value={regForm.player2FacebookLink} onChange={e => setRegForm({ ...regForm, player2FacebookLink: e.target.value })} onFocus={() => { setShowP2Dropdown(false); setShowP3Dropdown(false); }} required className={`h-11 pl-10 rounded-lg border-gray-200 focus:border-efb-red focus:bg-white transition-all text-sm ${selectedP2UserId ? 'bg-emerald-50/30' : 'bg-gray-50/50'}`} /></div></div>
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    {t.teamSize === 3 && (
                                        <div className="pt-3 pb-1 border-t border-gray-100 border-dashed space-y-4">
                                            ...
                                            <div className="grid grid-cols-1 gap-4">
                                                <div className="space-y-1.5 relative">
                                                    <Label className="text-xs font-medium text-gray-500">Tìm VĐV 3 trong hệ thống <span className="text-[10px] font-normal text-amber-500">(VĐV 3 bắt buộc có tài khoản)</span> <span className="text-red-400">*</span></Label>
                                                    <div className="relative">
                                                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                                                        <Input 
                                                            placeholder="Tìm bằng Tên hoặc Nickname..." 
                                                            value={showP3Dropdown ? p3SearchQuery : regForm.player3Name} 
                                                            onFocus={() => { if (!selectedP3UserId) { setShowP3Dropdown(true); setP3SearchQuery(regForm.player3Name); } }}
                                                            onChange={e => {
                                                                if (selectedP3UserId) return;
                                                                if (!showP3Dropdown) setShowP3Dropdown(true);
                                                                setP3SearchQuery(e.target.value);
                                                                setRegForm({ ...regForm, player3Name: e.target.value });
                                                            }} 
                                                            readOnly={!!selectedP3UserId}
                                                            required 
                                                            className={`h-11 pl-10 rounded-lg border-gray-200 focus:border-efb-red focus:bg-white transition-all text-sm ${selectedP3UserId ? 'bg-emerald-50/40 border-emerald-300 cursor-default' : 'bg-gray-50/50'}`} 
                                                        />
                                                        {isP3Searching && <Loader2 className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-emerald-500 animate-spin" />}
                                                        {selectedP3UserId && (
                                                            <button type="button" onClick={() => { setSelectedP3UserId(null); setRegForm({ ...regForm, player3Name: '', player3FacebookName: '', player3FacebookLink: '' }); setP3SearchQuery(''); setShowP3Dropdown(true); }} className="absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 rounded-full bg-red-100 hover:bg-red-200 flex items-center justify-center transition-colors" title="Xóa liên kết VĐV 3">
                                                                <X className="w-3 h-3 text-red-500" />
                                                            </button>
                                                        )}
                                                    </div>
                                                    
                                                    {showP3Dropdown && (p3SearchResults.length > 0 || p3SearchQuery.trim()) && (
                                                        <div className="absolute z-50 mt-1 w-full bg-white rounded-xl shadow-xl border border-gray-100 max-h-60 overflow-y-auto overflow-x-hidden">
                                                            {p3SearchResults.length > 0 ? (
                                                                <div className="p-1">
                                                                    {p3SearchResults.map((u: any) => (
                                                                        <button 
                                                                            key={u._id} 
                                                                            type="button"
                                                                            disabled={u.isRegistered}
                                                                            onClick={() => {
                                                                                ...
                                                                                setRegForm({
                                                                                    ...regForm,
                                                                                    player3Name: u.name || '',
                                                                                    player3FacebookName: u.facebookName || '',
                                                                                    player3FacebookLink: u.facebookLink || ''
                                                                                });
                                                                                setSelectedP3UserId(u._id);
                                                                                setP3SearchQuery(u.name);
                                                                                setShowP3Dropdown(false);
                                                                            }}
                                                                            className={`w-full text-left px-3 py-2.5 rounded-lg transition-colors flex items-center gap-3 ${u.isRegistered ? 'opacity-50 cursor-not-allowed bg-red-50/30' : 'hover:bg-gray-50'}`}
                                                                        >
                                                                            <img src={u.avatar || "https://ui-avatars.com/api/?name=" + encodeURIComponent(u.name)} alt="" className="w-8 h-8 rounded-full border border-gray-200" />
                                                            ...
                                                            ) : !isP3Searching && p3SearchQuery.trim() ? (
                                                                <div className="p-4 text-center">
                                                                    <div className="w-10 h-10 rounded-full bg-amber-100 flex items-center justify-center mx-auto mb-2">
                                                                        <AlertCircle className="w-5 h-5 text-amber-500" />
                                                                    </div>
                                                                    <p className="text-[12px] font-semibold text-gray-700">Không tìm thấy VĐV</p>
                                                                    <p className="text-[11px] text-gray-400 mt-1">VĐV 3 phải có tài khoản trên hệ thống.<br/>Vui lòng yêu cầu VĐV 3 đăng ký tài khoản trước.</p>
                                                                    <button type="button" onClick={() => setShowP3Dropdown(false)} className="mt-3 px-4 py-1.5 bg-gray-100 hover:bg-gray-200 rounded-lg text-[11px] font-medium transition-colors">Đóng</button>
                                                                </div>
                                                            ) : null}
                                                        </div>
                                                    )}
                                            ...
                                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                                <div className="space-y-1.5"><Label className="text-xs font-medium text-gray-500">Tên Facebook VĐV 3 <span className="text-red-400">*</span> {selectedP3UserId && <span className="text-[10px] text-emerald-500 font-normal">(Tự động điền)</span>}</Label><div className="relative"><Globe className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" /><Input placeholder="Tên Facebook" value={regForm.player3FacebookName} onChange={e => setRegForm({ ...regForm, player3FacebookName: e.target.value })} onFocus={() => { setShowP2Dropdown(false); setShowP3Dropdown(false); }} required className={`h-11 pl-10 rounded-lg border-gray-200 focus:border-efb-red focus:bg-white transition-all text-sm ${selectedP3UserId ? 'bg-emerald-50/30' : 'bg-gray-50/50'}`} /></div></div>
                                                <div className="space-y-1.5"><Label className="text-xs font-medium text-gray-500">Link Facebook VĐV 3 <span className="text-red-400">*</span> {selectedP3UserId && <span className="text-[10px] text-emerald-500 font-normal">(Tự động điền)</span>}</Label><div className="relative"><ExternalLink className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" /><Input placeholder="https://facebook.com/..." value={regForm.player3FacebookLink} onChange={e => setRegForm({ ...regForm, player3FacebookLink: e.target.value })} onFocus={() => { setShowP2Dropdown(false); setShowP3Dropdown(false); }} required className={`h-11 pl-10 rounded-lg border-gray-200 focus:border-efb-red focus:bg-white transition-all text-sm ${selectedP3UserId ? 'bg-emerald-50/30' : 'bg-gray-50/50'}`} /></div></div>
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    {t.teamSize >= 2 && (
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                                            <div className="space-y-1.5">
                                                <Label className="text-xs font-medium text-gray-500">Tên Team {t.teamSize === 6 ? <span className="text-red-400">*</span> : <span className="text-[10px] font-normal text-gray-400">(không bắt buộc)</span>}</Label>
                                                <Input placeholder="VD: FC Saigon, Team Hanoi..." value={regForm.teamName} onChange={e => setRegForm({ ...regForm, teamName: e.target.value })} className="h-11 rounded-lg border-gray-200 focus:border-efb-red bg-gray-50/50 focus:bg-white transition-all text-sm" maxLength={100} />
                                            </div>
                                        </div>
                                    )}
                            ...
                            {regStep === 3 && (
                                <motion.div key="s3" initial={{ opacity: 0, x: 15 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -15 }} className="space-y-4">
                                    ...
                                    <div className="p-4 rounded-xl bg-gray-50 border border-gray-100 space-y-2"><p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Tóm tắt</p><div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-sm"><div><span className="text-gray-400">Họ tên:</span> <span className="font-medium text-gray-700">{regForm.playerName}</span></div><div><span className="text-gray-400">SĐT:</span> <span className="font-medium text-gray-700">{regForm.phone}</span></div>{regForm.teamName && <div><span className="text-gray-400">Team:</span> <span className="font-medium text-gray-700">{regForm.teamName}</span></div>}{regForm.province && <div><span className="text-gray-400">Tỉnh/TP:</span> <span className="font-medium text-gray-700">{regForm.province}</span></div>}{(t.teamSize === 2 || t.teamSize === 3) && regForm.player2Name && <div><span className="text-gray-400">VĐV 2:</span> <span className="font-medium text-gray-700">{regForm.player2Name}</span></div>}{t.teamSize === 3 && regForm.player3Name && <div><span className="text-gray-400">VĐV 3:</span> <span className="font-medium text-gray-700">{regForm.player3Name}</span></div>}</div></div>
                                    <div className="pt-3 flex justify-between"><Button type="button" variant="outline" onClick={() => setRegStep(2)} className="h-11 px-6 rounded-lg font-medium border-gray-200 flex items-center gap-2"><ChevronLeft className="w-4 h-4" /> Quay lại</Button><Button type="submit" className="h-11 px-8 bg-gradient-to-r from-efb-red to-efb-red-light text-white rounded-lg font-medium shadow-sm flex items-center gap-2" disabled={isRegistering}>{isRegistering ? <Loader2 className="w-5 h-5 animate-spin" /> : <>Xác nhận <ChevronRight className="w-4 h-4" /></>}</Button></div>
                                </motion.div>
                            )}
                        </AnimatePresence>
                    ...
                    <div className="px-6 py-5 space-y-5">
                        ...
                        {myRegistration?.paymentStatus === 'paid' || myRegistration?.paymentStatus === 'confirmed' ? (
                            ...
                            <div className="p-4 rounded-xl bg-red-50 border border-red-200 flex items-center gap-3">
                                <Info className="w-6 h-6 text-efb-red flex-shrink-0" />
                                <div>
                                    <p className="text-sm font-medium text-efb-red">Giải đấu miễn phí</p>
                                    <p className="text-xs text-efb-red mt-0.5">Đăng ký của bạn đang chờ duyệt.</p>
                                </div>
                            </div>
                        ) : null}
                        ...
                        {myRegistration?.paymentStatus === 'unpaid' && t.entryFee > 0 && (
                            <>
                                {!selectedPayMethod ? (
                                        ...
                                        {paymentMethods.length === 0 ? (
                                            ...
                                            <div className="space-y-2">
                                                {paymentMethods.map((method: any) => (
                                                    <button key={method.id || method._id} type="button" onClick={() => handleSelectPaymentMethod(method)} disabled={isPaymentLoading} className="w-full flex items-center gap-3 p-3.5 rounded-xl border border-gray-200 hover:border-efb-red hover:bg-red-50/30 transition-all text-left group">
                                                        <div className="w-10 h-10 rounded-lg bg-gray-50 group-hover:bg-red-50 flex items-center justify-center flex-shrink-0 transition-colors">
                                                            {method.mode === 'auto' ? <Wallet className="w-5 h-5 text-efb-red" /> : <CreditCard className="w-5 h-5 text-gray-500 group-hover:text-efb-red" />}
                                                        </div>
                                                        <div className="flex-1 min-w-0">
                                                        ...
                                                        {isPaymentLoading ? <Loader2 className="w-4 h-4 animate-spin text-gray-400" /> : <ChevronRight className="w-4 h-4 text-gray-300 group-hover:text-efb-red transition-colors" />}
                                                    </button>
                                                ))}
                                            </div>
                                        )}
                                    ...
                                    <div className="space-y-4">
                                        <button type="button" onClick={() => setSelectedPayMethod(null)} className="flex items-center gap-1.5 text-sm text-efb-red hover:underline">
                                                ...
                                                <div className="space-y-4">
                                                    <div className="p-4 rounded-xl bg-gray-50 border border-gray-100 space-y-2.5">
                                                        ...
                                                        <div className="pt-1">
                                                            ...
                                                            <p className="text-sm font-medium text-efb-red select-all bg-red-50 px-2 py-1 rounded mt-1">{t.title} - {myRegistration?.playerName}</p>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div className="space-y-2.5">
                                                        ...
                                                        {paymentProofFile ? (
                                                            ...
                                                            <label className="cursor-pointer block">
                                                                <div className="flex items-center gap-3 p-4 rounded-xl border-2 border-dashed border-gray-200 hover:border-efb-red hover:bg-red-50/30 transition-all">
                                                                    <div className="w-10 h-10 rounded-lg bg-red-50 flex items-center justify-center"><Upload className="w-5 h-5 text-efb-red" /></div>
                                                                    <div>
                                                        ...
                                                        <Button onClick={handleSubmitPaymentProof} disabled={!paymentProofFile || isSubmittingProof} className="w-full h-11 bg-gradient-to-r from-efb-red to-efb-red-light text-white rounded-xl font-medium">
            ...
            <Dialog open={!!lineupViewTeam} onOpenChange={(open) => !open && setLineupViewTeam(null)}>
                <DialogContent className="max-w-lg bg-white rounded-2xl border-0 shadow-2xl p-0 overflow-hidden">
                    <div className="p-5 border-b border-gray-100 bg-gradient-to-br from-red-50/60 to-white">
                        <DialogTitle className="text-base font-bold text-gray-900 flex items-center gap-2">
                            <Eye className="w-4 h-4 text-efb-red" /> Đội hình thi đấu
                        </DialogTitle>
                        {lineupViewTeam && (
                            <div className="flex items-center gap-3 mt-3">
                                {lineupViewTeam.personalPhoto && (
                                    <img src={lineupViewTeam.personalPhoto} alt="" className="w-9 h-9 rounded-full object-cover border-2 border-red-200" />
                                )}
                                <div>
