...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!name || !email || !password) return apiError("Vui lòng điền đầy đủ thông tin", 400);
        if (password.length < 8) return apiError("Mật khẩu phải có ít nhất 8 ký tự", 400);
        if (password !== confirmPassword) return apiError("Mật khẩu xác nhận không khớp", 400);

        const existingUser = await User.findOne({ email: email.toLowerCase() }).select("+verificationCode +verificationCodeExpires");
        ...
        if (existingUser) {
            if (!existingUser.isVerified) {
                ...
                return apiResponse(
                    ...
                    200,
                    emailResult.success ? "Mã xác minh đã được gửi lại vào email" : "Tài khoản đã được cập nhật nhưng không gửi được email"
                );
            }
        ...
        await User.create({
            ...
            isVerified: false, verificationCode: code, verificationCodeExpires: codeExpires,
        });

        const emailResult = await sendVerificationEmail(email, name, code);
        return apiResponse(
            { email: email.toLowerCase(), requiresVerification: true, emailSent: emailResult.success },
            201,
            emailResult.success ? "Đăng ký thành công! Vui lòng kiểm tra email" : "Đăng ký thành công nhưng không gửi được email xác minh"
        );
    } catch (error: any) {
